...
> University project: NexusDB is a portable, fast database engine with SQL, indexes, foreign keys, and concurrency.

> Inspired by ISAM but modern. Goal: outperform MySQL on specific benchmarks.
...
|---|---|---|
| Engine language | **Rust** | No GC, memory control, maximum speed |
| Web mode | **MySQL wire protocol** | PHP/Python connect without custom drivers |
| Desktop mode | **Embedded library + C FFI** | In-process like SQLite, zero network latency |
| Storage | **mmap + 8KB pages** | Eliminates InnoDB double-buffering |
...
| Queries | **Vectorized + SIMD** | 10-50x on scans vs row-by-row |

---

## Usage Modes

### Server Mode — Web Applications

```
Web Client (PHP/Python/Node)
        │
        │ TCP :3306 (MySQL wire protocol)
        ▼
  Rust engine running as daemon
        │
        ▼
  file.db  /  file.wal
```
...
- Ideal for: REST APIs, web backend, microservices

### Embedded Mode — Desktop Applications

```
Application (C++/Python/Java/Electron)
        │
        │ direct function call (C FFI / native binding)
        ▼
  Rust engine in-process (like SQLite)
        │
        ▼
  file.db  /  file.wal  (local)
```

- The engine lives inside the app's process
- Zero network latency (direct in-memory calls)
- No daemon, no port, no extra installation
- Ideal for: Electron apps, desktop C++/Qt, CLI tools, Python scripts

### Mode Comparison

| Feature | Server Mode | Embedded Mode |
|---|---|---|
| Latency | ~0.1ms (TCP loopback) | ~1µs (in-process) |
| Multiple processes | Yes | No (one process at a time) |
| Installation | Daemon + port | Just the library (.so/.dll) |
| Ideal for | Web, APIs, microservices | Desktop, CLI, scripts |

---

## Compatible Clients (no custom drivers)

```
PHP    → PDO::mysql or mysqli        (point to :3306)
Python → PyMySQL or mysql-connector  (point to :3306)
GUI    → MySQL Workbench, DBeaver   (same)
```

The server implements the **MySQL wire protocol**, so any existing MySQL client
connects directly without modifications.
...
│                     │               │                        │
│  Tokio TCP listener │               │  lib.rs exposes API    │
│  MySQL handshake    │               │  C FFI (#[no_mangle])  │
│  Basic auth         │               │  No network, no daemon │
└──────────┬──────────┘               └───────────┬────────────┘
           │                                      │
           └──────────────┬───────────────────────┘
           ...
           │              STORAGE ENGINE                     │
           │                                                 │
           │  Copy-on-Write B+ Tree  (MVCC without locks)    │
           │  mmap of .db file       (zero double-buffer)    │
           │  WAL append-only        (no double-write)       │
           │  FK Checker             (reverse index)         │
           └─────────────────────────────────────────────────┘
                          │
                 ┌────────▼────────┐
                 │  table.db       │  ← mmap pages
                 │  table.wal      │  ← sequential log
                 │  table.idx      │  ← B+ Tree per index
                 └─────────────────┘
```

---

## Embedded Mode — Desktop API
...
│   ├── server.rs       ← server mode entry point (TCP + MySQL protocol)
│   ├── engine/         ← shared engine (storage, parser, executor)
│   └── ffi.rs          ← exported C bindings
```

### Native Rust API (for Rust/Electron apps via Neon)
...
impl Database {
    /// Open or create a DB on disk
    pub fn open(path: &str) -> Result<Self, DbError> { ... }

    /// Open in-memory DB (tests, temporary CLI)
...
### C FFI (for C++, Qt, Java JNI, Python ctypes)
...
pub extern "C" fn dbyo_execute(
    ...
    // returns 0 = OK, error code if it fails
    ...
}
...
### Cargo.toml — compile as library AND binary
...
### Usage from different languages (embedded mode)
...
## Why MySQL is Slow (and How We Address It)

### Problem 1: Double-Buffering

```
MySQL InnoDB:
  Disk → OS Page Cache → InnoDB Buffer Pool → Query
            (copy 1)          (copy 2)

  Same data TWICE in RAM.
  Buffer Pool can be 80% of the server and still duplicates.

Our DB (mmap):
  Disk → OS Page Cache → Query
            (copy 1, direct)

  The OS already caches. We don't reinvent the buffer pool.
```

### Problem 2: InnoDB Double-Write Buffer

```
MySQL writes each page TWICE:
  Write 1 → doublewrite buffer (special disk area)
  Write 2 → actual position in .ibd

With WAL + per-page checksum this is unnecessary.
We eliminate it completely.
```

### Problem 3: MVCC with Separate Undo Log

```
MySQL:
  New version → B+ Tree
  Old version → undo log (ibdata1) → additional I/O for historical reads

Our DB (Copy-on-Write B+ Tree):
  Readers have a pointer to the old root
  Old versions are pages of the tree itself
  Zero additional I/O for concurrent reads
```

### Problem 4: Thread-per-connection

```
MySQL:
  1000 connections = 1000 OS threads
  Each thread = ~8MB stack = 8GB just in stacks
  Massive context switching

Our DB (Tokio async):
  1000 connections = ~8 real OS threads
  Multiplexed with async/await
  No unnecessary context switching
```

### Problem 5: Row-by-row execution without SIMD

```
MySQL evaluates WHERE row by row:
  for row in table:
    if row.age > 25: emit(row)     ← scalar, 1 at a time

Our DB (vectorized + SIMD AVX2):
  load 256 age values into AVX2 registers
  compare 256 values with ONE instruction
  bit mask → emit in batch

  ~16x faster on table scans
```
...
### Complete table of supported types

| SQL Type | Alias | Bytes | Internal Rust | Notes |
|---|---|---|---|---|
| `BOOL` | `BOOLEAN` | 1 | `bool` | |
| `TINYINT` | `INT1` | 1 | `i8` | -128 to 127 |
| `UTINYINT` | `UINT1` | 1 | `u8` | 0 to 255 |
| `SMALLINT` | `INT2` | 2 | `i16` | |
| `USMALLINT` | `UINT2` | 2 | `u16` | |
| `INT` | `INTEGER, INT4` | 4 | `i32` | |
| `UINT` | `UINT4` | 4 | `u32` | |
| `BIGINT` | `INT8` | 8 | `i64` | |
| `UBIGINT` | `UINT8` | 8 | `u64` | internal LSN, page_id |
| `HUGEINT` | `INT16` | 16 | `i128` | cryptography, checksums |
| `REAL` | `FLOAT4, FLOAT` | 4 | `f32` | coordinates, ratings |
| `DOUBLE` | `FLOAT8, DOUBLE PRECISION` | 8 | `f64` | scientific calculations |
| `DECIMAL(p,s)` | `NUMERIC(p,s)` | 16 | `rust_decimal::Decimal` | money, accounting |
| `CHAR(n)` | | n | `[u8; n]` | fixed length |
| `VARCHAR(n)` | | ≤n | `CompactStr` | maximum n chars |
| `TEXT` | | var | `CompactStr` / TOAST | unlimited |
| `CITEXT` | | var | `CompactStr` | case-insensitive |
| `BYTEA` | `BLOB, BYTES` | var | `Arc<[u8]>` | binary, TOAST if >2KB |
| `BIT(n)` | | n bits | `BitVec` | exactly n bits |
| `VARBIT(n)` | | ≤n bits | `BitVec` | up to n bits |
| `DATE` | | 4 | `time::Date` | date only |
| `TIME` | | 8 | `time::Time` | time only |
| `TIMETZ` | | 12 | `time::Time + offset` | time with timezone |
| `TIMESTAMP` | | 8 | `i64` µs | without timezone (local) |
| `TIMESTAMPTZ` | | 8 | `i64` µs UTC | **recommended** |
| `INTERVAL` | | 16 | `DbInterval` | months + days + µs |
| `UUID` | | 16 | `[u8; 16]` | v4 random, v7 sortable |
| `INET` | | 16 | `std::net::IpAddr` | IPv4 and IPv6 |
| `CIDR` | | 17 | `DbCidr` | IP network with prefix |
| `MACADDR` | | 6 | `[u8; 6]` | MAC address |
| `JSON` | `JSONB` | var | `Arc<serde_json::Value>` | TOAST if >2KB |
| `VECTOR(n)` | | 4n | `Arc<Vec<f32>>` | AI embeddings |
| `T[]` | | var | `Arc<Vec<Value>>` | array of any type |
| `ENUM` | | 1-4 | `u32` (index) | validated on insert |
| `RANGE(T)` | | var | `Range<T>` | int4range, daterange, tsrange |
| `COMPOSITE` | | var | `Vec<Value>` | CREATE TYPE … AS (…) |
| `DOMAIN` | | = base | = base | CHECK on base type |

---

### Internal Value enum — compact representation (24 bytes)

```rust
use compact_str::CompactStr;  // strings ≤23 chars on stack, no heap
use std::sync::Arc;
...
enum Value {
    ...
    Float(f32),                         //  4 bytes
    Double(f64),                        //  8 bytes
    Decimal(rust_decimal::Decimal),     // 16 bytes — exact

    // Text
    Text(CompactStr),                   // 24 bytes stack if ≤23 chars
    Bytes(Arc<[u8]>),                   // zero-copy with Arc

    // Date/time
    Date(i32),                          //  4 bytes — days since epoch
    Time(i64),                          //  8 bytes — µs since midnight
    Timestamp(i64),                     //  8 bytes — µs since UTC epoch
    Interval(i32, i32, i64),            // 16 bytes — months, days, µs

    // Special types
    Uuid([u8; 16]),                     // 16 bytes — NOT as String
    Inet(std::net::IpAddr),             // 16 bytes — IPv4 and IPv6
    MacAddr([u8; 6]),                   //  6 bytes

    // Collections (shared with Arc for zero-copy)
    Array(Arc<Vec<Value>>),
    Json(Arc<serde_json::Value>),
    Vector(Arc<Vec<f32>>),              // AI embeddings

    // Range
    Range(Arc<(Option<Value>, Option<Value>, bool, bool)>),
}
// Total size: 24 bytes thanks to CompactStr + Arc for large values
```

---

### NULL bitmap — no Option<T> per value

```rust
// BAD: Option<T> adds 8 bytes per nullable field
struct RowNaive {
    id:    Option<i32>,   // 8 bytes
    age:   Option<i32>,   // 8 bytes
    score: Option<f64>,   // 16 bytes
}  // total: 32 bytes for 3 simple values

// GOOD: 1 bit per column in the row header
#[repr(C)]
struct Row {
    null_bitmap: u64,      // bit i = 1 → column i is NULL
    data:        [u8; N],  // values packed without Option
}
...
// Savings: 7 bytes × nullable_columns per row
```

---

### Column Encoding — transparent compression

```rust
enum ColumnEncoding {
    /// No encoding — for high cardinality (UUIDs, hashes)
    Plain,

    /// Dictionary: value → u8/u16 index
    /// Ideal for ENUMs, status, country (≤256 distinct values)
    /// Typical savings: 'pending'(7 bytes) → 0u8(1 byte) = 7x
    Dictionary {
        dict: Vec<Value>,
        codes: Vec<u8>,   // u8 if ≤256 values, u16 if ≤65536
    },

    /// Run-Length Encoding: (value, count)
    /// Ideal for sorted data with many repeated values
    /// Ex: [A,A,A,A,B,B,C] → [(A,4),(B,2),(C,1)]
    RunLength {
        runs: Vec<(Value, u32)>,
    },

    /// Delta: store differences between consecutive values
    /// Ideal for timestamps, sequential IDs
    /// Ex: [1000,1001,1002,1005] → [1000, +1, +1, +3]
    Delta {
        base:   i64,
        deltas: Vec<i16>,   // small differences in 2 bytes
    },

    /// BitPacking: n bits per value (for integers in small range)
    /// Ex: years 2020-2030 → 4 bits each → 2 values per byte
    BitPacking {
        bits_per_value: u8,
        data:           Vec<u8>,
    },

    /// Frame Of Reference: base + small delta
    /// Ideal for timestamps within a time window
    FrameOfReference {
        base:   i64,
        deltas: Vec<i32>,
    },
...
### Exact DECIMAL — never use FLOAT for money
...
assert_ne!(price, 0.3);  // 0.30000000000000004 — ERROR!

// With DECIMAL:
let price = dec!(0.1) + dec!(0.2);
assert_eq!(price, dec!(0.3));  // always exact

// Safe financial operations
let subtotal = dec!(19.99) * dec!(3);          // 59.97
let tax      = subtotal * dec!(0.19);          // 11.3943
let total    = subtotal + tax.round_dp(2);     // 71.37
```

---

### TIMESTAMPTZ — always UTC internally

```rust
// The problem with TIMESTAMP without timezone:
// Server in UTC+0 → client in UTC-5 → they insert "2026-03-21 10:00"
// What time was it? Impossible to know.

// TIMESTAMPTZ: always UTC on disk, converts when displaying
use time::{OffsetDateTime, UtcOffset};

fn store_timestamptz(dt: OffsetDateTime) -> i64 {
    dt.unix_timestamp_nanos() / 1000  // store in µs UTC
}

fn display_timestamptz(micros: i64, client_tz: UtcOffset) -> OffsetDateTime {
    OffsetDateTime::from_unix_timestamp(micros / 1_000_000)
        .unwrap()
        .to_offset(client_tz)  // convert to client timezone when displaying
}
```

---

### UUID v7 — better than v4 for PKs

```rust
// UUID v4: completely random → inserts at random positions in B+ Tree
// → many page splits → fragmentation → more I/O

// UUID v7: timestamp (48 bits) + random (80 bits) → sortable by time
// → nearly sequential inserts → fewer splits → better performance as PK

use uuid::Uuid;

fn gen_pk() -> [u8; 16] {
    Uuid::now_v7().into_bytes()
    // first 48 bits = ms timestamp → sortable
    // last 80 bits = random → unique
}

// Performance comparison with 1M inserts:
// UUID v4 PK: ~150k inserts/s (many splits)
// UUID v7 PK: ~250k inserts/s (nearly sequential inserts)
// BIGINT PK:  ~280k inserts/s (completely sequential)
```

---

### INTERVAL — correct duration

```rust
// Why INTERVAL needs months + days + µs separately:
// '1 month' is not a fixed 30 days (January=31, February=28/29)
// '1 day' is not a fixed 86400 seconds (DST can add/remove 1h)

struct DbInterval {
    months: i32,   // years*12 + months
    days:   i32,   // exact days
    micros: i64,   // hours*3.6B + minutes*60M + seconds*1M + µs
}
...
impl DbInterval {
    fn add_to_timestamp(&self, ts: OffsetDateTime) -> OffsetDateTime {
        ts.checked_add(Duration::days(self.days as i64))
          ...
          .unwrap()
        // months: use calendar (not fixed arithmetic)
    }
}
```
...
### RANGE types — overlaps and containment

```sql
-- Integer range
SELECT '[1,10)'::int4range @> 5;      -- true  (5 is in [1,10))
SELECT '[1,5)' && '[3,8)'::int4range; -- true  (they overlap)

-- Reservations without overlap (exclusion constraint)
CREATE TABLE rooms (
  room_id  INT,
  period   TSRANGE,
  EXCLUDE USING gist(room_id WITH =, period WITH &&)
  -- two reservations of the same room cannot overlap
);

INSERT INTO rooms VALUES (1, '[2026-03-21 09:00, 2026-03-21 11:00)');
INSERT INTO rooms VALUES (1, '[2026-03-21 10:00, 2026-03-21 12:00)');
-- ERROR: exclusion conflict (period overlaps)
```
...
### 1. VarInt Encoding — variable-size integers

Instead of always reserving 8 bytes for BIGINT, use only the bytes needed.
90% of IDs and counters are small — immediate savings.

```
Value         Bytes used    Overhead vs fixed BIGINT
──────────────────────────────────────────────────
0 – 127            1 byte        -87%
...
Typical ID (42) → 1 byte instead of 8 = 87% savings on IDs
```
...
fn encode_varint(mut val: u64, buf: &mut Vec<u8>) {
    loop {
        ...
        buf.push(byte | 0x80);       // more bytes: high bit = 1
    }
}
...
// Zigzag for signed integers (prevents negatives from using 9 bytes)
...
### 2. DECIMAL Fast Path — no heap for ≤18 digits

```rust
// Most monetary values fit in i64 with fixed scale.
// Only escalate to BigDecimal if >18 digits are really needed.

enum Decimal {
    // Fast path: pure integer arithmetic, no heap, SIMD-friendly
    // Covers DECIMAL(1,0) through DECIMAL(18,18)
    Small {
        mantissa: i64,  // 19.99 with scale=2 → 1999
        scale:    u8,   // number of decimal places (0-18)
    },
    // Slow path: only for DECIMAL(>18, x)
...
impl Decimal {
    fn add(&self, other: &Self) -> Self {
        match (self, other) {
            // Same scale: direct i64 addition (1 CPU instruction)
            (Small { mantissa: a, scale: sa }, Small { mantissa: b, scale: sb }) if sa == sb => {
                Small { mantissa: a + b, scale: *sa }
            }
            // Different scales: align first
            (Small { mantissa: a, scale: sa }, Small { mantissa: b, scale: sb }) => {
                let (a2, b2, scale) = align_scales(*a, *sa, *b, *sb);
                Small { mantissa: a2 + b2, scale }
            }
            // Either is Big: convert both
            _ => Big(Box::new(self.to_big() + other.to_big())),
        }
    }
}
// Benchmark: Decimal::Small addition is ~20x faster than full rust_decimal
```

---

### 3. JSONB — pre-parsed binary format

JSON text re-parses on every access. JSONB stores in binary with an offset table.

```
JSON text:  '{"age":25,"name":"Ana","role":"admin"}'
             → parse O(n) on each query = slow

JSONB binary:
  [header: 3 keys] [offset_table: age@12, name@16, role@24]
  [keys sorted: "age\0name\0role\0"]
  [values: 25][3:"Ana"][5:"admin"]
  → field access: binary search over keys = O(log k) without parsing = fast
```

```rust
struct JsonbHeader {
    n_keys:  u32,
    offsets: Vec<u32>,   // position of each key in the buffer
}
...
fn json_to_jsonb(json: &serde_json::Value) -> Vec<u8> {
    ...
    keys.sort();   // sort keys for binary search
    // serialize header + offsets + keys + values
    serialize_jsonb(keys, json)
}
// Individual field access: 10-50x faster than JSON text
```

---

### 4. VECTOR — quantization to save space

```sql
-- full f32: 4 bytes/dim — maximum precision
CREATE TABLE docs (embedding VECTOR(1536));               -- 6,144 bytes/row

-- f16 half precision: 2 bytes/dim — <1% loss in cosine similarity
CREATE TABLE docs (embedding VECTOR(1536) PRECISION float16);  -- 3,072 bytes/row

-- int8 quantized: 1 byte/dim — ~2-3% loss — ultra-fast with SIMD
CREATE TABLE docs (embedding VECTOR(1536) PRECISION int8);     -- 1,536 bytes/row
```

```rust
// Scalar quantization: map f32 [-1.0, 1.0] → i8 [-128, 127]
fn quantize_f32_to_i8(vec: &[f32]) -> (Vec<i8>, f32) {
    ...
    (quantized, scale)   // store scale factor for dequantization
}
...
// Cosine similarity with int8: AVX2 processes 32 values per instruction
// vs f32: only 8 values per instruction → 4x faster in search
...
### 5. SIMD per type — choose the minimum appropriate type

```
Type       Values per AVX2 instruction   When to use
──────────────────────────────────────────────────────────────────
TINYINT     32 values                   age, rating, numeric status
SMALLINT    16 values                   year, code, small quantity
INT          8 values                   normal ID, price in cents
BIGINT       4 values                   high-scale ID, timestamp
REAL         8 values (FMA)             coordinates, ML scores
DOUBLE       4 values (FMA)             scientific calculations

→ Use TINYINT instead of INT when data fits = 4x faster on scans
→ The query planner can suggest the minimum type when running ANALYZE
```

```rust
// The planner detects if an INT column could be TINYINT
...
### 6. PAX Layout — columnar within each page

Better cache locality for analytics without changing the file format.

```
Row layout (traditional):
  Page: [id1|age1|name1] [id2|age2|name2] [id3|age3|name3]
  SELECT AVG(age): also reads id+name even though not needed

PAX (Partition Attributes Across):
  Page: [ids: 1|2|3|...] [ages: 25|30|22|...] [names: "Ana"|"Bob"|...]
  SELECT AVG(age): only reads the ages block → 3x less I/O
  SIMD on ages: all i32 contiguous → process 8 at a time
```

```rust
struct PaxPage {
    header:  PageHeader,
    // columns stored contiguously within the page
...
struct ColumnSlice {
    ...
    data:     Range<usize>,   // offset within the page buffer
}

// Read only the 'age' column without touching the others
...
### 7. Per-column statistics — smart planner

```rust
#[derive(Debug, Clone)]
struct ColumnStats {
    // Basic
    null_frac:     f64,              // 0.0 to 1.0 — fraction of NULLs
    n_distinct:    i64,              // >0 = exact count, -1 = all distinct
    avg_width:     u32,              // average bytes (useful for TEXT)
    row_count:     u64,

    // Distribution
    min:           Value,
    max:           Value,
    histogram:     Vec<Value>,       // N equal-frequency buckets
    most_common:   Vec<(Value,f64)>, // [(value, frequency)] top-K

    // For the index planner
    correlation:   f64,              // -1.0 to 1.0: how sorted the data is
    // correlation ≈ 1.0  → index scan very efficient (data nearly sorted)
    // correlation ≈ 0.0  → index scan expensive (random I/O), prefer seq scan
}

// Usage in the planner:
// WHERE age BETWEEN 20 AND 30
// → histogram says 15% selectivity
// → correlation(age) = 0.95 → index scan (data nearly sorted by age)
// → estimate: 150K rows × 0.15 = 22.5K rows → index scan

// UPDATE to thousands of rows with correlation ≈ 0 → seq scan cheaper
...
ANALYZE users;
ANALYZE users (age, email);   -- specific columns only
...
### 8. Collation — complete system

#### The real problem

```sql
-- Without correct collation, these queries fail silently:
SELECT * FROM users WHERE name = 'jose';    -- does NOT find 'José'
SELECT * FROM users WHERE email LIKE 'ana%';  -- does NOT find 'Ana@gmail.com'
SELECT * FROM users ORDER BY name;          -- 'Álvarez' appears after 'Zuniga'
SELECT COUNT(*) FROM users GROUP BY name;   -- 'jose' and 'José' are two distinct groups
UPPER('josé')    -- gives 'JOSé' without Unicode (the é is not converted)
LENGTH('José')   -- can give 4 or 5 depending on Unicode normalization
```

#### Unicode CLDR levels

```
Level 1 — Primary:     ignores accents AND case  → a = á = A = Á
Level 2 — Secondary:   distinguishes accents, not case → a = A, a ≠ á
Level 3 — Tertiary:    distinguishes accents AND case → a ≠ A, a ≠ á  (default)
Level 4 — Quaternary:  distinguishes everything + punctuation and spaces
```
...
_bin  → exact binary         (byte by byte)

-- Common combinations
es_419_ci    → Latin American Spanish, case-insensitive
es_419_ci_ai → Latin American Spanish, ignoring case and accents
utf8_bin     → binary, maximum precision (for emails, tokens, hashes)
en_US_ci     → US English, case-insensitive
```
...
CREATE TABLE users (
  id     INT PRIMARY KEY,
  name   TEXT,              -- inherits es_419_ci from the DB
  email  TEXT COLLATE utf8_bin,     -- override: exact binary
  notes  TEXT COLLATE es_419_ci_ai  -- override: also ignores accents
) COLLATE es_419_ci;

-- Level 4: individual column (higher priority)
ALTER TABLE users MODIFY email TEXT COLLATE utf8_bin;

-- Level 5: query (highest priority of all)
SELECT * FROM users WHERE name COLLATE es_419_ci = 'jose';
-- finds: 'Jose', 'JOSE', 'José', 'josé', 'josè'
```
...
```sql
SHOW CHARACTER SETS;
-- utf8mb4   → full UTF-8 (1-4 bytes, emojis included) ← recommended
-- utf8      → UTF-8 limited to 3 bytes (no emojis — legacy)
-- latin1    → ISO-8859-1 (Western Europe — legacy)
-- ascii     → 7 bits (English only)
-- utf16     → UTF-16 (Windows/Java legacy)
-- binary    → raw bytes

-- Always UTF-8 internally (native Rust)
-- Automatic conversion in wire protocol if client requests another encoding
```

#### Unicode Normalization — the hidden problem

```
The 'é' can be represented in TWO valid Unicode forms:
  NFC:  U+00E9           → a single codepoint "precomposed é"
  NFD:  U+0065 + U+0301  → "e" + combining accent (two codepoints)

They are the same visual character but different bytes:
  'é'(NFC) == 'é'(NFD)  → FALSE without normalizing
  LENGTH('é' NFC) = 1, LENGTH('é' NFD) = 2

Solution: always normalize to NFC before storing
```
...
fn normalize_for_storage(s: &str) -> String {
    s.nfc().collect()      // NFC: canonical composed form
}

fn normalize_for_search(s: &str) -> String {
    s.nfkc().collect()     // NFKC: also flattens ligatures (ﬁ→fi, ™→TM, ½→1/2)
}

fn strip_accents(s: &str) -> String {
    // NFD separates character + accent, then filter combining marks
    s.nfd()
...
Fase 10 — Modo embebido + FFI       (semana 15-16)
  ...
  ✓ [MOBILE] flutter_rust_bridge: bindings Dart generados desde la API Rust de nexusdb-embedded
  ...
  ✓ [DESKTOP] Tauri plugin: nexusdb-embedded como plugin Tauri — produce .msi/.dmg/.AppImage con nexusdb dentro
  ✓ [DESKTOP] npm package nexusdb-node: wheels precompilados por plataforma (win32-x64, darwin-arm64, linux-x64)
...
Fase 15 — MongoDB + DoltDB + Arrow  (semana 25-26)
  ...
  ✓ Kafka source: CREATE KAFKA SOURCE ... — consumir topics de Kafka como tablas de NexusDB
...
Fase 19 — Mantenimiento + observabilidad  (semana 34-35)
  ...
  ✓ Prometheus endpoint: GET /metrics — métricas en formato Prometheus text exposition
...
Fase 20 — Tipos + importación/exportación  (semana 36-37)
  ...
  ✓ Backup incremental y restore completo  ✓ CREATE RECURSIVE VIEW — sintaxis SQL:1999 para vistas sobre CTEs recursivos (azúcar sobre WITH RECURSIVE)
  ✓ WITH CHECK OPTION — valida que INSERT/UPDATE sobre una view actualizable cumpla la condición WHERE de la view
  ✓ WITH CASCADED CHECK OPTION — propaga la validación a todas las views base en cadena
...
Fase 21 — SQL avanzado              (semana 38-39)
  ...
  ✓ [MOBILE] DELETE reescrito a UPDATE SET deleted_at = NOW() en tablas con soft_delete
  ✓ [MOBILE] SELECT agrega WHERE deleted_at IS NULL implícitamente en tablas con soft_delete
  ✓ [MOBILE] SELECT ... WITH DELETED para ver filas eliminadas (sync, auditoría)
  ✓ [MOBILE] HARD DELETE ... para borrado físico explícito cuando se requiere
  ✓ Temporal tables (SQL:2011): PERIOD FOR SYSTEM_TIME, GENERATED ALWAYS AS ROW START/END
  ✓ WITH (SYSTEM_VERSIONING = ON) — engine trackea valid_from/valid_to automáticamente en cada UPDATE/DELETE
  ✓ FOR SYSTEM_TIME AS OF <timestamp> — viajar al estado exacto de la tabla en cualquier momento
  ✓ FOR SYSTEM_TIME BETWEEN t1 AND t2 — rango histórico consultable con SQL estándar
  ✓ FOR SYSTEM_TIME ALL — ver todas las versiones de todas las filas
  ✓ Bitemporal: application-time period + system-time period en la misma tabla
  ✓ Historia almacenada en páginas separadas con compresión automática (LZ4) para no inflar la tabla principal
...
Fase 22 — Features de producto      (semana 40-42)
  ...
  ✓ Auto REST API (nexusdb-rest, PostgREST-style): puerto :3310, schema autodescubierto
  ✓ REST CRUD automático: GET/POST/PATCH/DELETE /rest/<tabla> sin escribir código
  ✓ REST filtros: ?column=eq.valor&order=col.desc&limit=10&offset=20
  ✓ REST joins: ?select=*,orders(*) — expand relaciones por FK automáticamente
  ✓ REST + RLS: JWT claim → RLS policy → cada usuario ve solo sus datos
  ✓ REST bulk operations: POST con array JSON → INSERT múltiple en una transacción
...
Fase 23 — Retrocompatibilidad       (semana 43-45)
  ...
  ✓ dbt schema tests: not_null, unique, accepted_values, relationships — ejecutan sobre NexusDB
  ✓ dbt docs generate: documenta el schema de NexusDB automáticamente
  ✓ dbt seeds: cargar CSV como tablas en NexusDB desde el CLI de dbt
...
Fase 24 — Sistema de tipos completo (semana 46-48)
  ...
  ✓ MACADDR8 — EUI-64 de 8 bytes ([u8; 8]), estándar IEEE moderno, extiende MACADDR
  ✓ UHUGEINT — entero sin signo de 128 bits (u128), extiende HUGEINT
  ✓ MONEY — tipo monetario locale-aware distinto de DECIMAL: símbolo, redondeo fijo, lc_monetary
  ✓ XML — tipo SQL estándar (SQL:2003), almacenamiento TOAST, validación well-formed en insert
  ✓ MAP(K, V) — diccionario con tipos fijos de key y value, más eficiente que JSONB para KV plano
  ✓ MULTIRANGE — array de rangos: int4multirange, datemultirange, tsmultirange (extiende RANGE)
...
Fase 27 — Query Optimizer real      (semana 55-57)
  ...
  ✓ Adaptive Query Execution (AQE): re-optimización del plan durante la ejecución basada en estadísticas reales
  ✓ AQE — Adaptive Join: cambia Hash Join → Nested Loop si una tabla resulta más pequeña de lo estimado
  ✓ AQE — Skew Join: detecta y redistribuye particiones con datos sesgados (hot keys) automáticamente
  ✓ AQE — Partition coalescing: combina particiones pequeñas post-shuffle para reducir overhead
  ✓ AQE — Cardinality feedback: corrige estadísticas con rows reales observados para mejorar futuros planes
  ✓ AQE — Runtime filter: genera bloom filters en runtime para eliminar particiones irrelevantes en JOINs
...
Fase 28 — Completitud SQL           (semana 58-60)
  ...
  ✓ ROWVERSION — contador atómico de versión por fila para optimistic locking sin timestamps
...
Fase 29 — Funciones completas       (semana 61-63)
  ...
  ✓ Funciones XML: xmlparse(), xmlserialize(), xpath(), xmlagg(), xml_is_well_formed()
  ✓ Funciones HSTORE: hstore(), hstore_to_json(), each(), keys(), values(), operadores ?& ?|
  ✓ Funciones MONEY: to_char(money, format), SET lc_monetary para símbolo por sesión
  ✓ Funciones MAP: map_keys(), map_values(), map_entries(), map_from_entries(), element_at()
  ✓ HyperLogLog (HLL): tipo de columna HLL para COUNT DISTINCT aproximado con <1% error y <1KB storage
  ✓ HLL_ADD(), HLL_COUNT(), HLL_MERGE() — acumular y combinar sketches de múltiples shards
  ✓ Count-Min Sketch (CMS): CMS_ADD(), CMS_FREQUENCY() — frecuencia aproximada de elementos en streams
  ✓ T-Digest: TDIGEST_ADD(), TDIGEST_PERCENTILE(col, 0.99) — percentiles exactos sin guardar todos los valores
  ✓ Bloom filter como tipo de columna: BLOOM(capacity, error_rate) — membership queries O(1) con falsos positivos configurables
  ✓ Sketches combinables: HLL_MERGE(a, b) permite calcular DISTINCT sobre particiones en paralelo y unir resultados
...
Fase 30 — Infraestructura pro       (semana 64-66)
  ...
  ✓ LTREE — tipo jerárquico de etiquetas (Electronics.Phones.Android), operadores <@ @> ~ ?
  ✓ Índice GiST para LTREE (usa GiST de esta misma fase), operadores con índice en árboles profundos
  ✓ Índice GIN para MAP(K,V) y HSTORE — búsqueda por key y por value con O(log n)
  ✓ Funciones LTREE: subpath(), nlevel(), text2ltree(), ltree2text(), lca() (lowest common ancestor)
  ✓ Transaction-level connection multiplexing (pgBouncer-style integrado): 10.000 clientes → 50 conexiones reales
  ✓ Multiplexing modes: session (1:1), transaction (retorna al pool entre transacciones), statement
  ✓ Server-side prepared statements preservados en modo transaction multiplexing
  ✓ SET / RESET de parámetros de sesión replicados al reconectar (search_path, timezone, etc.)
  ✓ Stats: nexusdb_pool_stats vista con conexiones activas, idle, waiting, canceladas por timeout
...
Fase 31 — Features finales           (semana 67-69)
  ...
  ✓ Geospatial: tipos DDL formales GEOMETRY(subtype, SRID): GEOMETRY(POINT,4326), GEOMETRY(POLYGON), LINESTRING, MULTIPOINT, GEOMETRYCOLLECTION — compatibles con WKT/WKB
  ✓ ST_* functions: ST_DISTANCE_KM, ST_CONTAINS, ST_INTERSECTS, ST_WITHIN, ST_AREA, ST_GEOMFROMTEXT
  ✓ Índice R-Tree (rstar) para todos los subtipos GEOMETRY
...
Fase 34 — Infraestructura distribuida y completitud  (semana 77-80)
  ...
  ✓ Arrow Flight — do_put(ticket): ingest masivo de datos columnar desde Python/Spark/Flink
  ✓ Delta Lake: lectura nativa de tablas Delta (S3/GCS/local) con time travel y schema evolution
  ✓ Apache Iceberg: lectura nativa de tablas Iceberg con snapshot queries y partition pruning
  ✓ COPY ... TO ... WITH FORMAT DELTA/ICEBERG — exportar tablas en formato open lakehouse
  ✓ Multi-tenancy automatizado: CREATE TENANT con schema template, quotas y aislamiento
  ✓ Quotas por tenant: storage, connections, max_rows, max_query_time — enforcement automático
  ✓ BACKUP TENANT / RESTORE TENANT — backup y restore aislado por tenant sin afectar otros
  ✓ ALTER TENANT ALL APPLY MIGRATION — aplicar migraciones de schema a todos los tenants en paralelo
  ✓ Tenant billing view: nexusdb_tenant_usage con storage, queries, rows_read por tenant
...
Fase 35 — Deployment y DevEx        (semana 81-83)
  ...
  ✓ [DESKTOP/WIN] winget package: winget install nexusdb.NexusDB
  ...
  ✓ [DESKTOP/MAC] .dmg para modo servidor local: instala nexusdb-server + LaunchAgent (arranque automático)
  ...
  ✓ nexusdb-odbc: driver ODBC escrito en Rust (odbc-api crate) compilado como cdylib
  ...
  ✓ Se conecta internamente vía PostgreSQL wire protocol — reutiliza nexusdb-network sin duplicar lógica
  ...
  ✓ Paquete .deb (Ubuntu/Debian): nexusdb-server_X.Y.Z_amd64.deb — instala binario, usuario sistema, servicio systemd
  ✓ Paquete .rpm (RHEL/CentOS/Fedora/AlmaLinux): nexusdb-server-X.Y.Z.x86_64.rpm — equivalente para distros RPM
  ...
  ✓ nexusdb-wizard: wizard interactivo de configuración por terminal (ratatui + dialoguer)
      Paso 1 — Bienvenida: versión detectada, modo de instalación (nuevo / reconfigurar)
      Paso 2 — Directorio de datos: ruta con validación de permisos y espacio disponible
      Paso 3 — Puertos: MySQL (def. 3306) y PostgreSQL (def. 5432) con detección de conflictos
      Paso 4 — Admin: usuario admin + contraseña con confirmación y medidor de fortaleza
      Paso 5 — TLS: ninguno / auto-signed / certificado propio (pide rutas cert/key)
      Paso 6 — Memoria: RAM detectada automáticamente, slider para asignar porcentaje a NexusDB
  ...
  ✓ nexusdb-wizard --reconfigure: relanza el wizard sobre una instalación existente sin perder datos
  ✓ nexusdb-wizard --non-interactive: modo CI con flags (--data-dir --port --password --no-tls)
  ✓ [STUDIO] NexusDB Studio: web UI servida por el propio servidor en puerto :8080 — sin instalación extra
  ✓ [STUDIO] Dashboard: queries/s, latencia p50/p95/p99, conexiones activas, hit rate de cache, WAL lag
  ✓ [STUDIO] Table editor: ver, filtrar, editar y eliminar filas con UI visual — sin escribir SQL
  ✓ [STUDIO] SQL editor: autocomplete de tablas/columnas/funciones, syntax highlighting, múltiples tabs
  ✓ [STUDIO] EXPLAIN visualizer: árbol del plan de ejecución con costos y tiempos reales por nodo
  ✓ [STUDIO] Schema designer: crear/alterar tablas con UI drag & drop, visualización de FKs como grafo
  ✓ [STUDIO] Migration manager: historial de migraciones con diff de schema entre versiones
  ✓ [STUDIO] User & roles manager: crear usuarios, asignar roles, configurar RLS policies visualmente
  ✓ [STUDIO] Replication monitor: lag por réplica, slots activos, publicaciones y suscripciones
  ✓ [STUDIO] AI assistant integrado: NL_QUERY() desde la UI — preguntar en lenguaje natural
  ✓ [STUDIO] Construido con Axum (backend) + HTMX + Tailwind (frontend) — sin framework JS pesado
  ✓ [STUDIO] Autenticación: usa las credenciales existentes de NexusDB — sin usuario separado
  ✓ [STUDIO] nexusdb-studio: crate separado, opcional, activado con --studio en nexusdb-server
  ✓ [VSCODE] nexusdb-vscode: extensión publicada en VS Code Marketplace
  ✓ [VSCODE] Schema explorer: árbol de conexiones → bases de datos → tablas → columnas/índices/FK
  ✓ [VSCODE] IntelliSense: autocomplete de tablas, columnas, tipos propios (VECTOR, LTREE, HLL, MAP...)
  ✓ [VSCODE] Syntax highlighting: keywords NexusDB, operadores <=> <-> <#>, funciones AI_*
  ...
  ✓ Kubernetes operator: CRD NexusDBCluster — gestiona deploy, scaling, backups y upgrades en K8s
  ...
  ✓ ActiveRecord adapter: gem nexusdb-activerecord — Rails conecta directamente a NexusDB
...
Fase 36 — Mobile Sync Layer         (semana 84-87)
  ✓ nexusdb-sync: nuevo crate (depende de nexusdb-wal, nexusdb-mvcc, nexusdb-replication, nexusdb-catalog)
  ✓ HLC (Hybrid Logical Clock) por fila: timestamp físico + contador lógico, sin coordinador central
  ✓ Columna __hlc TIMESTAMPTZ GENERATED ALWAYS AS HLC() STORED — automática en tablas sync-enabled
  ✓ Columna __deleted BOOLEAN DEFAULT FALSE — soft delete integrado con sync (usa Fase 21)
  ✓ Replication slots como base del delta sync: WAL retenido hasta que el cliente confirme (usa Fase 31)
  ✓ Delta sync protocol: GET /sync?table=X&since=<lsn> — retorna solo filas modificadas desde LSN dado
  ✓ Conflict resolution trait: trait ConflictResolver { fn resolve(local: Row, remote: Row) -> Row }
  ✓ Estrategias built-in: LastWriteWins (HLC), FieldLevelMerge (por columna), ServerWins, ClientWins
  ✓ CRDT types como feature flag en nexusdb-types: LWWRegister<T>, GCounter, PNCounter, ORSet<T>
  ✓ CREATE TABLE ... WITH (sync = true, conflict = 'last_write_wins') — sintaxis SQL unificada
  ✓ SyncManager: WalSubscriber que trackea posición por cliente y retiene WAL necesario
  ✓ nexusdb-embedded opcional: feature "mobile-sync" activa nexusdb-sync en la build
  ✓ Dart binding (flutter_rust_bridge): sync_pull(since_lsn) y sync_push(changes) como futures
  ✓ Tests: simular dos dispositivos offline, modificar mismas filas, sincronizar y verificar resolución

Fase 37 — Graph Layer                (semana 88-91)
  ✓ nexusdb-graph: nuevo crate para datos de grafos sobre el storage existente (B+ Tree + páginas)
  ✓ Tipos DDL: CREATE GRAPH, CREATE NODE TABLE, CREATE EDGE TABLE con propiedades tipadas
  ✓ SQL/PGQ (ISO SQL:2023): MATCH (a:Persona)-[r:AMIGO_DE]->(b:Persona) dentro de SELECT
  ✓ Graph traversal: BFS, DFS, shortest path (Dijkstra, A*), all paths entre dos nodos
  ✓ Graph algorithms: PageRank, HITS, community detection (Louvain), betweenness centrality
  ✓ Índice de adyacencia: acceso O(1) a neighbors de un nodo sin full scan
  ✓ Hybrid queries: JOIN entre tablas relacionales y grafos en la misma query
  ✓ Property graph model: nodos y edges con propiedades arbitrarias tipadas (MAP internamente)
  ✓ GRAPH_TABLE() función de tabla: expone resultados de traversal como filas relacionales
  ✓ Casos de uso: redes sociales, detección de fraude (ciclos entre cuentas), recomendaciones, knowledge graphs
```
...
### Desktop Deployment — Embedded vs. Server

Dos modelos de distribución completamente distintos:

```
EMBEDDED (como SQLite)                   SERVER LOCAL (como MySQL local)
─────────────────────────────────────    ──────────────────────────────────
nexusdb vive DENTRO de la app            nexusdb se instala SEPARADO
El usuario solo instala "MiApp"          El usuario instala "NexusDB"
...
#### Windows — embedded en app propia
...
```powershell
# WinGet
winget install nexusdb.NexusDB

# Chocolatey
choco install nexusdb

# MSI directo
msiexec /i nexusdb-1.0.0-x86_64.msi

# → instala nexusdb-server.exe como Windows Service
...
xcrun notarytool submit MiApp.zip --wait   # notarización Apple
```
...
#### Linux — embedded en AppImage

```bash
# AppImage: un solo archivo, corre en cualquier distro sin instalar
...
#### Tauri — el caso ideal (Rust nativo, multiplataforma)

```toml
# Cargo.toml de la app Tauri
[dependencies]
nexusdb-embedded = "1.0"   # se compila estáticamente en la app
tauri = { version = "2", features = ["shell-open"] }
```

```bash
# Un solo comando genera instaladores para las 3 plataformas:
cargo tauri build

# Produce automáticamente:
# Windows  → src-tauri/target/release/bundle/msi/MiApp_1.0.0_x64.msi
#             src-tauri/target/release/bundle/nsis/MiApp_1.0.0_x64-setup.exe
# macOS    → src-tauri/target/release/bundle/dmg/MiApp_1.0.0_x64.dmg
#             src-tauri/target/release/bundle/macos/MiApp.app
# Linux    → src-tauri/target/release/bundle/appimage/MiApp_1.0.0_amd64.AppImage
#             src-tauri/target/release/bundle/deb/MiApp_1.0.0_amd64.deb
#
# En TODOS: nexusdb-embedded compilado estáticamente — el usuario
# nunca instala NexusDB por separado.
```

#### Electron — npm package con binarios nativos

```json
{ "dependencies": { "nexusdb-node": "^1.0" } }
```

```bash
# electron-builder empaqueta automáticamente para 3 plataformas:
npx electron-builder --win --mac --linux

# nexusdb-node descarga el binario correcto según plataforma:
# node_modules/nexusdb-node/
# ├── nexusdb-win32-x64.node
# ├── nexusdb-darwin-arm64.node
# ├── nexusdb-darwin-x64.node
# └── nexusdb-linux-x64.node
```

### Installation Wizard — nexusdb-wizard

Terminal wizard built with `ratatui` (TUI framework) + `dialoguer` (prompts) + `indicatif` (progress bars).

```
╔══════════════════════════════════════════════════════════╗
║           NexusDB Installation Wizard v1.0               ║
║                                                          ║
║  Welcome! This wizard will configure your NexusDB        ║
║  installation. You can re-run it anytime with:           ║
║    nexusdb-wizard --reconfigure                          ║
╚══════════════════════════════════════════════════════════╝

  [1/10] Installation mode
  > Fresh install
  ...
  [3/10] Ports
  MySQL-compatible port  : 3306  [✓ available]
  PostgreSQL-compatible  : 5432  [✓ available]

  [4/10] Admin credentials
  Username : admin
  Password : ████████  [Strength: Strong ✓]
  Confirm  : ████████  [Match ✓]

  [5/10] TLS
  > None (localhost only — not recommended for production)
    Auto-generate self-signed certificate
    Use existing certificate (provide paths)

  [6/10] Memory allocation
  Detected RAM: 32 GB
  Allocate to NexusDB: [████████████░░░░] 50% (16 GB)
  ← → to adjust, ↵ to confirm
  ...
  ✓ Port 3306 responding
  ✓ Admin login OK

  Installation complete! Connect with:
    psql -h localhost -p 5432 -U admin
    mysql -h localhost -P 3306 -u admin -p
    DBeaver → New connection → PostgreSQL → localhost:5432
```

```rust
// nexusdb-wizard/src/main.rs — structure
// Crates: ratatui, dialoguer, indicatif, sysinfo, nix
...
struct WizardState {
    ...
    tls_mode:      TlsMode,
    memory_pct:    u8,
    repl_role:     ReplicationRole,
}

// Each step is a function that mutates WizardState
    ...
    let input: String = Input::with_theme(&ColorfulTheme::default())
        ...
        .validate_with(|p: &String| {
            ...
            if let Ok(space) = available_space(path) {
                if space < 1_000_000_000 { return Err("Less than 1GB available"); }
            }
            Ok(())
        })
        .interact_text()?;
    state.data_dir = PathBuf::from(input);
    Ok(())
}

fn step_ports(state: &mut WizardState) -> Result<()> {
    state.mysql_port = Input::new().with_prompt("MySQL port")
        .default(3306).interact()?;
    // check if port is free
    if is_port_in_use(state.mysql_port) {
        eprintln!("⚠ Port {} is already in use", state.mysql_port);
    }
    Ok(())
}

fn step_apply(state: &WizardState) -> Result<()> {
    let pb = ProgressBar::new(6);
    create_dir_all(&state.data_dir)?;        pb.inc(1);
    write_config(state)?;                    pb.inc(1);
    create_system_user("nexusdb")?;          pb.inc(1);
...
Script de instalación universal (`install.sh`):
...
install_package
nexusdb-wizard   # launch wizard immediately after install
```
