...
public final class AxiomDB {
    ...
    @discardableResult
    public func execute(_ sql: String) throws -> Int64 {
        guard let p = ptr else { throw AxiomError(message: "database is closed") }
        let n = axiomdb_execute(p, sql)
        if n < 0 { throw AxiomError(message: lastError() ?? "execute failed") }
        return n
    }

    /// Executes a SELECT, returning rows as arrays (fastest).
    public func queryTuples(_ sql: String) throws -> [[AxiomValue]] {
        try queryPacked(sql).rows
    }

    /// Executes a SELECT, returning column names plus rows.
    public func queryWithColumns(_ sql: String) throws -> (columns: [String], rows: [[AxiomValue]]) {
        try queryPacked(sql)
    }

    /// Executes a SELECT, returning rows as dictionaries (column name → value).
    public func query(_ sql: String) throws -> [[String: AxiomValue]] {
        let (cols, rows) = try queryPacked(sql)
    ...
    private func queryPacked(_ sql: String) throws -> (columns: [String], rows: [[AxiomValue]]) {
        guard let p = ptr else { throw AxiomError(message: "database is closed") }
        var len: size_t = 0
        guard let buf = axiomdb_query_packed(p, sql, &len) else {
