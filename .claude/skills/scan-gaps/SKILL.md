...
## Scan checklists by area

### Parser scan checklist

```
DML:
  [ ] SELECT with window functions (OVER, PARTITION BY, ROW_NUMBER)
  [ ] Common Table Expressions (WITH ... AS)
  [ ] UNION / INTERSECT / EXCEPT (with and without ALL)
  [ ] INSERT ... ON DUPLICATE KEY UPDATE
  [ ] REPLACE INTO
  [ ] Multi-table DELETE (DELETE t1 FROM t1 JOIN t2)
  [ ] Multi-table UPDATE (UPDATE t1 JOIN t2 SET ...)
  [ ] MERGE / UPSERT
...
DDL:
  ...
  [ ] ALTER TABLE CHANGE (MySQL-specific rename+retype)
...
Expressions:
  ...
  [ ] CAST(x AS type)
  [ ] String functions: CONCAT, SUBSTRING, TRIM, REPLACE, UPPER, LOWER
  [ ] Date functions: NOW(), CURDATE(), DATE_ADD, DATEDIFF
  [ ] Aggregate: COUNT, SUM, AVG, MIN, MAX, GROUP_CONCAT
  [ ] Subquery in SELECT list
  [ ] Subquery in FROM (derived table)
  [ ] Subquery in WHERE (IN, EXISTS, scalar)
  [ ] ANY/SOME/ALL with subquery
```

### Wire protocol scan checklist

```
  [ ] COM_QUERY — basic text protocol
  [ ] COM_STMT_PREPARE / EXECUTE / CLOSE — binary protocol
  [ ] COM_PING
  [ ] COM_INIT_DB (USE database)
  [ ] COM_FIELD_LIST
  [ ] COM_STATISTICS
  [ ] Correct column type codes in result set
  [ ] Correct character set negotiation
  [ ] Multi-statement support (CLIENT_MULTI_STATEMENTS)
  [ ] SSL/TLS handshake
  [ ] Authentication plugins (mysql_native_password, caching_sha2)
  [ ] Server status flags (autocommit, in_transaction, etc.)
```

### Type system scan checklist

```
  [ ] INT → BIGINT implicit widening in comparisons
  [ ] Text → INT/BIGINT coercion in WHERE
  [ ] Text → TIMESTAMP coercion in INSERT
  [ ] NULL handling in all operators
  [ ] DECIMAL precision and scale propagation
  [ ] DATE vs TIMESTAMP distinction
  [ ] BOOLEAN vs TINYINT equivalence (MySQL)
  [ ] ENUM type
  [ ] SET type
  [ ] JSON vs JSONB coercion
```
