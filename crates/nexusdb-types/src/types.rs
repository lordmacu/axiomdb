...
//!
//! [`DataType`] is the in-memory type descriptor used by `nexusdb-types` and
//! `nexusdb-sql`. It is intentionally separate from [`ColumnType`] in
//! `nexusdb-catalog`, which is a compact `repr(u8)` enum for disk storage.
//! The executor converts between the two when reading column definitions from
//! the catalog.
//!
//! [`ColumnType`]: nexusdb_catalog::schema::ColumnType
