//! Parser comparison benchmark — AxiomDB vs sqlparser-rs.
//!
...
fn bench_comparison(c: &mut Criterion) {
    ...
    for (name, sql) in [
        ...
        // AxiomDB parser
        group.bench_function("axiomdb", |b| {
            b.iter(|| axiomdb_sql::parse(black_box(sql), None).unwrap())
        });
...
fn bench_throughput(c: &mut Criterion) {
    ...
    for n_queries in [1_000u64, 10_000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("axiomdb_simple_select", n_queries),
            &n_queries,
            |b, &n| {
                b.iter(|| {
                    for _ in 0..n {
                        black_box(axiomdb_sql::parse(SIMPLE_SELECT, None).unwrap());
                    }
                })
            },
        );
