...
use axiomdb_core::{DbError, TransactionSnapshot};
use axiomdb_sql::{analyze, parse};
...
impl Fixture {
    ...
    fn analyze(&self, sql: &str) -> Result<axiomdb_sql::ast::Stmt, DbError> {
...
fn test_single_table_resolves_col_idx() {
    ...
    if let axiomdb_sql::ast::Stmt::Select(s) = stmt {
        // WHERE: age > 18 — age is col 2
        if let Some(axiomdb_sql::expr::Expr::BinaryOp { left, .. }) = s.where_clause {
            if let axiomdb_sql::expr::Expr::Column { col_idx, name } = *left {
        ...
        if let axiomdb_sql::ast::SelectItem::Expr {
...
fn test_join_resolves_combined_col_idx() {
    ...
    if let axiomdb_sql::ast::Stmt::Select(s) = stmt {
        ...
        if let axiomdb_sql::ast::SelectItem::Expr {
            expr: axiomdb_sql::expr::Expr::Column { col_idx, .. },
            ..
        } = &s.columns[0]
        {
        ...
        if let axiomdb_sql::ast::JoinCondition::On(axiomdb_sql::expr::Expr::BinaryOp {
            left,
            right,
            ..
        }) = &s.joins[0].condition
        {
            if let axiomdb_sql::expr::Expr::Column { col_idx, .. } = left.as_ref() {
                assert_eq!(*col_idx, 0, "u.id should be at combined index 0");
            }
            if let axiomdb_sql::expr::Expr::Column { col_idx, .. } = right.as_ref() {
...
fn test_drop_table_unknown_with_if_exists_ok() {
    ...
    assert!(matches!(stmt, axiomdb_sql::ast::Stmt::DropTable(_)));
}
...
fn test_insert_valid_column_list_ok() {
    ...
    let stmt = f
        ...
        .unwrap();
    assert!(matches!(stmt, axiomdb_sql::ast::Stmt::Insert(_)));
}
...
fn test_update_resolves_where_col_idx() {
    ...
    let stmt = f
        ...
        .unwrap();
    if let axiomdb_sql::ast::Stmt::Update(u) = stmt {
        if let Some(axiomdb_sql::expr::Expr::BinaryOp { left, .. }) = u.where_clause {
            if let axiomdb_sql::expr::Expr::Column { col_idx, .. } = *left {
                assert_eq!(col_idx, 2, "age should be at index 2");
            }
        }
    }
...
fn test_delete_resolves_where_col_idx() {
    ...
    if let axiomdb_sql::ast::Stmt::Delete(d) = stmt {
        if let Some(axiomdb_sql::expr::Expr::BinaryOp { left, .. }) = d.where_clause {
            if let axiomdb_sql::expr::Expr::Column { col_idx, .. } = *left {
...
fn test_transaction_stmts_pass_through() {
    ...
    assert!(matches!(
        ...
        axiomdb_sql::ast::Stmt::Begin
    ));
    assert!(matches!(
        f.analyze("COMMIT").unwrap(),
        axiomdb_sql::ast::Stmt::Commit
    ));
    assert!(matches!(
        f.analyze("ROLLBACK").unwrap(),
        axiomdb_sql::ast::Stmt::Rollback
    ));
}
