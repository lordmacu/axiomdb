...
fn insert_batch_count_and_aggregates_are_exact() {
    ...
    let count = scalar_int(ok("SELECT COUNT(*) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert_eq!(count, N as i64, "COUNT(*) mismatch after COMMIT");

    // SUM(id) = N*(N+1)/2 (Gauss formula).
    let expected_sum = (N as i64) * (N as i64 + 1) / 2;
    let sum_id = scalar_int(ok("SELECT SUM(id) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert_eq!(sum_id, expected_sum, "SUM(id) mismatch");

    // MIN(age): age = 18 + (i % 62). For i=62, age=18 → minimum.
    let min_age = scalar_int(ok("SELECT MIN(age) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert_eq!(min_age, 18, "MIN(age) should be 18");

    // MAX(age): age = 18 + 61 = 79.
    let max_age = scalar_int(ok("SELECT MAX(age) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    ...
    let min_score = scalar_real(ok("SELECT MIN(score) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert!((min_score - 100.0).abs() < 0.001, "MIN(score) should be ~100.0, got {min_score}");

    // MAX(score): i=999 → 100.0 + 999*0.1 = 199.9.
    let max_score = scalar_real(ok("SELECT MAX(score) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert!((max_score - 199.9).abs() < 0.001, "MAX(score) should be ~199.9, got {max_score}");
}
...
fn insert_batch_spot_check_row_values() {
    ...
    for &id in &[1usize, 100, 500, 999, 1000, 1001, 1999] {
        ...
        if let Value::Real(got) = row[4] {
            assert!((got - expected_score).abs() < 0.001, "score mismatch at id={id}: expected {expected_score}, got {got}");
        } else {
...
fn insert_batch_rollback_to_savepoint_preserves_pre_savepoint_rows() {
    ...
    ok("ROLLBACK TO SAVEPOINT sp", &mut s, &mut txn, &mut bloom, &mut ctx);

    ok("COMMIT", &mut s, &mut txn, &mut bloom, &mut ctx);

    // Only BEFORE rows must survive.
    let count = scalar_int(ok("SELECT COUNT(*) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert_eq!(count, BEFORE as i64, "expected {BEFORE} rows after rollback to savepoint, got {count}");

    // The last pre-savepoint row must exist.
    let last = scalar_int(ok("SELECT MAX(id) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert_eq!(last, BEFORE as i64, "MAX(id) should be {BEFORE}, got {last}");
...
fn insert_batch_two_consecutive_transactions_accumulate_correctly() {
    ...
    let count = scalar_int(ok("SELECT COUNT(*) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert_eq!(count, total as i64, "total count after two txns should be {total}");

    let expected_sum = (total as i64) * (total as i64 + 1) / 2;
    let sum_id = scalar_int(ok("SELECT SUM(id) FROM bench_users", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert_eq!(sum_id, expected_sum, "SUM(id) after two txns should be {expected_sum}");
}
...
fn insert_batch_different_tables_in_same_txn_do_not_cross_contaminate() {
    ...
    for i in 1..=500i32 {
        ok(&format!("INSERT INTO t1 VALUES ({i}, {})", i * 10), &mut s, &mut txn, &mut bloom, &mut ctx);
    }
    for i in 1..=300i32 {
        ok(&format!("INSERT INTO t2 VALUES ({i}, {})", i * 20), &mut s, &mut txn, &mut bloom, &mut ctx);
    }
    for i in 501..=600i32 {
        ok(&format!("INSERT INTO t1 VALUES ({i}, {})", i * 10), &mut s, &mut txn, &mut bloom, &mut ctx);
    }
    ...
    let c1 = scalar_int(ok("SELECT COUNT(*) FROM t1", &mut s, &mut txn, &mut bloom, &mut ctx));
    let c2 = scalar_int(ok("SELECT COUNT(*) FROM t2", &mut s, &mut txn, &mut bloom, &mut ctx));
    assert_eq!(c1, 600, "t1 must have 600 rows, got {c1}");
    assert_eq!(c2, 300, "t2 must have 300 rows, got {c2}");

    // Sum check for t1: (1..=600) * 10
    let sum1 = scalar_int(ok("SELECT SUM(val) FROM t1", &mut s, &mut txn, &mut bloom, &mut ctx));
    let expected1: i64 = (1..=600i64).map(|i| i * 10).sum();
    assert_eq!(sum1, expected1, "SUM(val) for t1 mismatch");

    // Sum check for t2: (1..=300) * 20
    let sum2 = scalar_int(ok("SELECT SUM(val) FROM t2", &mut s, &mut txn, &mut bloom, &mut ctx));
