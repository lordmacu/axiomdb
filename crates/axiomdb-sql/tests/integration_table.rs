...
fn test_table_engine_empty_scan() {
    ...
    let rows = TableEngine::scan_table(&mut storage, &table_def, &columns, snap).unwrap();
...
fn test_table_engine_insert_mvcc_visibility() {
    ...
    let rows_old =
        TableEngine::scan_table(&mut storage, &table_def, &columns, snap_before).unwrap();
    assert_eq!(rows_old.len(), 0, "snapshot before insert must see 0 rows");

    // Fresh snapshot sees 1 row.
    let snap_after = txn.snapshot();
    let rows_new = TableEngine::scan_table(&mut storage, &table_def, &columns, snap_after).unwrap();
...
fn test_table_engine_scan_respects_snapshot() {
    ...
    let rows = TableEngine::scan_table(&mut storage, &table_def, &columns, snap_after_a).unwrap();
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].1[0], Value::Int(1));

    // Current snapshot sees both.
    let snap_current = txn.snapshot();
    let rows2 = TableEngine::scan_table(&mut storage, &table_def, &columns, snap_current).unwrap();
