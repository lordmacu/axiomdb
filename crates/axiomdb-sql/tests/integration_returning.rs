//! Phase 21.4 / 21.4b — `RETURNING` clause for INSERT, UPDATE, DELETE.
...
#[test]
fn insert_returning_star_multiple_rows() {
    let (mut s, mut t, mut b, mut c) = setup();
    seed(&mut s, &mut t, &mut b, &mut c);
    let r = rows(
        "INSERT INTO t VALUES (10, 'x'), (11, 'y'), (12, 'z') RETURNING *",
        &mut s,
        &mut t,
        &mut b,
        &mut c,
    ...
    assert_eq!(r.len(), 3);
    assert_eq!(r[0].len(), 2);
    assert_eq!(as_i64(&r[2][0]), 12);
    assert_eq!(as_text(&r[2][1]), "z");
}

#[test]
fn insert_returning_id_auto_increment() {
    let (mut s, mut t, mut b, mut c) = setup();
    run_ctx(
        "CREATE TABLE u (id INT AUTO_INCREMENT PRIMARY KEY, email TEXT)",
        &mut s,
        &mut t,
        &mut b,
        &mut c,
    ...
    .unwrap();
    let r = rows(
        "INSERT INTO u (email) VALUES ('a@x') RETURNING id",
        &mut s,
        &mut t,
        &mut b,
        &mut c,
...
#[test]
fn insert_returning_expression_alias() {
    let (mut s, mut t, mut b, mut c) = setup();
    seed(&mut s, &mut t, &mut b, &mut c);
    let r = rows(
        "INSERT INTO t VALUES (5, 'q') RETURNING id * 2 AS doubled",
        &mut s,
        &mut t,
        &mut b,
        &mut c,
...
#[test]
fn insert_without_returning_still_returns_affected() {
    let (mut s, mut t, mut b, mut c) = setup();
    seed(&mut s, &mut t, &mut b, &mut c);
    let res = run_ctx(
        "INSERT INTO t VALUES (99, 'k')",
        &mut s,
        &mut t,
        &mut b,
        &mut c,
...
// ── UPDATE RETURNING (Phase 21.4b) ─────────────────────────────────────────

#[test]
fn update_returning_post_update_values() {
    let (mut s, mut t, mut b, mut c) = setup();
    seed(&mut s, &mut t, &mut b, &mut c);
    let r = rows(
...
fn update_returning_star() {
    ...
    seed(&mut s, &mut t, &mut b, &mut c);
    let r = rows(
        "UPDATE t SET name = 'Z' WHERE id >= 2 RETURNING *",
        &mut s,
        &mut t,
        &mut b,
        &mut c,
...
#[test]
fn update_returning_empty_result_on_no_match() {
    let (mut s, mut t, mut b, mut c) = setup();
    seed(&mut s, &mut t, &mut b, &mut c);
    let r = rows(
        "UPDATE t SET name = 'never' WHERE id = 999 RETURNING *",
        &mut s,
        &mut t,
        &mut b,
        &mut c,
...
#[test]
fn update_without_returning_still_returns_affected() {
    let (mut s, mut t, mut b, mut c) = setup();
    seed(&mut s, &mut t, &mut b, &mut c);
    let res = run_ctx(
        "UPDATE t SET name = 'p' WHERE id = 1",
        &mut s,
        &mut t,
        &mut b,
        &mut c,
