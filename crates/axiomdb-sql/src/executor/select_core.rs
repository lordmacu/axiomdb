fn execute_select(
    ...
    // Phase 20.10 — GENERATE_SERIES TVF in FROM.
...
/// Phase 20.10 — SELECT whose FROM is `GENERATE_SERIES(start, stop [, step])`.
/// Materializes the integer or date sequence, then applies WHERE / GROUP BY /
/// ORDER BY / LIMIT exactly like the UNNEST path.
fn execute_select_generate_series_source(
    mut stmt: SelectStmt,
    storage: &dyn StorageEngine,
    txn: &TxnManager,
    _conn_txn: Option<&ConnectionTxn>,
) -> Result<QueryResult, DbError> {
    let gs = match stmt.from.take() {
        Some(FromClause::GenerateSeries(gs)) => *gs,
        _ => unreachable!(
            "execute_select_generate_series_source called with non-GenerateSeries FROM"
        ),
    };

    let alias = gs.alias.clone().unwrap_or_else(|| "generate_series".to_string());
    let derived_rows = crate::generate_series::materialize_from_clause(&gs, &[])?;
    let derived_cols = crate::generate_series::column_metas_for_generate_series(&gs);
