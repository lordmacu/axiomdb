...
fn sql(db: &mut Db, q: &str) {
    db.run(q).unwrap_or_else(|e| panic!("SQL failed: {q}\nError: {e:?}"));
}

fn qrows(db: &mut Db, q: &str) -> Vec<Vec<Value>> {
    db.query(q).unwrap_or_else(|e| panic!("query failed: {q}\nError: {e:?}"))
}

fn scalar_int(db: &mut Db, q: &str) -> i64 {
    match qrows(db, q).into_iter().next().and_then(|r| r.into_iter().next()) {
...
fn create_clustered_table(db: &mut Db) {
    ...
    sql(db, "CREATE TABLE bench_users (
        id     INT  NOT NULL,
        name   TEXT NOT NULL,
        age    INT  NOT NULL,
...
fn create_heap_table(db: &mut Db) {
    ...
    sql(db, "CREATE TABLE bench_users_heap (
        id     INT  NOT NULL,
        name   TEXT NOT NULL,
        age    INT  NOT NULL,
        active BOOL NOT NULL,
...
fn appender_clustered_count_aggregates_and_spot_checks_correct() {
    ...
    let active = scalar_int(&mut db, "SELECT COUNT(*) FROM bench_users WHERE active = TRUE");
    ...
    if let Value::Real(v) = min_score_rows[0][0] {
        assert!((v - expected_min_score).abs() < 0.001,
    ...
    if let Value::Real(v) = max_score_rows[0][0] {
        assert!((v - expected_max_score).abs() < 0.001,
            "MAX(score) must be ~{expected_max_score:.1}, got {v}");
    }

    // Spot-check specific rows via PK lookup.
    for &id in &[1usize, 50, 100, 200, 299, 300] {
        let result = qrows(&mut db, &format!(
            "SELECT id, name, age, active, score, email FROM bench_users WHERE id = {id}"
        ));
...
fn appender_heap_count_aggregates_and_spot_checks_correct() {
    ...
    assert_eq!(sum_id, (N as i64) * (N as i64 + 1) / 2, "SUM(id) mismatch on heap");
    ...
    let active = scalar_int(&mut db, "SELECT COUNT(*) FROM bench_users_heap WHERE active = TRUE");
...
fn appender_large_multi_flush_all_rows_correct() {
    ...
    let sum_id = scalar_int(&mut db, "SELECT SUM(id) FROM bench_users");
    assert_eq!(sum_id, expected_sum, "SUM(id) must match Gauss formula after multi-flush");

    // MIN score=100.0 at id=1000 (and 2000, 3000, etc.), MAX=199.9 at id=999.
    let min_score_rows = qrows(&mut db, "SELECT MIN(score) FROM bench_users");
    if let Value::Real(v) = min_score_rows[0][0] {
        assert!((v - 100.0).abs() < 0.001, "MIN(score) must be ~100.0 after multi-flush");
    }
    let max_score_rows = qrows(&mut db, "SELECT MAX(score) FROM bench_users");
    if let Value::Real(v) = max_score_rows[0][0] {
        assert!((v - 199.9).abs() < 0.001, "MAX(score) must be ~199.9 after multi-flush");
    }

    // Spot-check rows at flush-boundary positions (multiples of 1024 ± 1).
    for &id in &[1, 1023, 1024, 1025, 2047, 2048, 2049, 3071, 3072, 3073, 4095, 4096, 4097, 5000] {
        let result = qrows(&mut db, &format!(
            "SELECT id, name, age, active, score, email FROM bench_users WHERE id = {id}"
        ));
...
fn appender_indexed_secondary_index_lookup_correct() {
    ...
    let ghost = qrows(&mut db, "SELECT id FROM bench_users WHERE email = 'ghost@b.local'");
    assert_eq!(ghost.len(), 0, "index lookup for missing email must return 0 rows");

    // Aggregates must still be correct.
    let sum_id = scalar_int(&mut db, "SELECT SUM(id) FROM bench_users");
    assert_eq!(sum_id, (N as i64) * (N as i64 + 1) / 2, "SUM(id) mismatch with secondary index");
}
...
fn appender_typed_builder_inserts_correct_values() {
    ...
    sql(&mut db, "CREATE TABLE bench_users_heap (
        id     INT  NOT NULL,
        name   TEXT NOT NULL,
        age    INT  NOT NULL,
        active BOOL NOT NULL,
    ...
    let committed = app.finish().expect("appender finish");
    assert_eq!(committed, N as u64, "typed builder finish must report {N} committed rows");
...
fn create_index_after_appender_load_is_consistent() {
    ...
    assert_eq!(count_after, count_before, "COUNT(*) must not change after CREATE INDEX");
    assert_eq!(sum_after, sum_before, "SUM(id) must not change after CREATE INDEX");
    ...
    for &id in &[1usize, 100, 500, 1000] {
        ...
        assert_eq!(result.len(), 1, "post-index lookup email='{email}' must return 1 row");
        check_row(&result[0], id);
    }
}
...
fn crud_flow_via_appender_insert_scan_delete_correct() {
    ...
    let empty = qrows(&mut db,
        "SELECT id FROM bench_users");
