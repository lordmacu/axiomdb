...
use nexusdb_core::{RecordId, TransactionSnapshot};
use nexusdb_sql::TableEngine;
...
fn test_table_engine_delete_invalid_slot_error() {
    ...
    assert!(
        matches!(err, nexusdb_core::error::DbError::InvalidSlot { .. }),
...
fn test_table_engine_insert_outside_txn_error() {
    ...
    .unwrap_err();
    assert!(
        matches!(err, nexusdb_core::error::DbError::NoActiveTransaction),
...
fn create_table_helper(
    ...
    let snap = txn.snapshot();
    let reader = nexusdb_catalog::CatalogReader::new(storage, snap).unwrap();
    reader
