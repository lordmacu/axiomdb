...
//! Integration tests here can import both `nexusdb-wal` (TxnManager,
//! CrashRecovery) and `nexusdb-storage` (MmapStorage, IntegrityChecker)
...
use nexusdb_wal::{CrashRecovery, RecoveryResult, TxnManager};
...
fn test_corrupt_checkpoint_lsn_documented_failure_mode() {
    ...
    // Corrupt the checkpoint_lsn in the meta page to a huge value.
    {
        ...
        nexusdb_storage::write_checkpoint_lsn(&mut storage, 999_999).unwrap();
        storage.flush().unwrap();
    }
