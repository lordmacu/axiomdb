...
use nexusdb_core::{error::DbError, TransactionSnapshot, TxnId};
...
mod tests {
    use super::*;
    use nexusdb_storage::Page;
    use nexusdb_storage::{insert_tuple, read_tuple, MemoryStorage, PageType};
    ...
    fn test_rollback_undo_delete_clears_deletion() {
        ...
        let txn2 = mgr.begin().unwrap();
        {
            ...
            nexusdb_storage::delete_tuple(&mut p, slot_id, txn2).unwrap();
    ...
    fn test_rollback_undo_update() {
        ...
        let txn2 = mgr.begin().unwrap();
        {
            ...
            let mut p = Page::from_bytes(bytes).unwrap();
            let new_slot =
                nexusdb_storage::update_tuple(&mut p, old_slot, b"updated", txn2).unwrap();
            storage.write_page(page_id, &p).unwrap();
            mgr.record_update(
                1,
                b"k",
