...
use nexusdb_core::TransactionSnapshot;
...
fn test_delete_index() {
    ...
    let reader1 = CatalogReader::new(&storage, snap1).unwrap();
    ...
    let reader2 = CatalogReader::new(&storage, snap2).unwrap();
...
fn test_delete_table_cascades_columns_and_indexes() {
    ...
    let snap1 = committed_snap(&txn);
    let r1 = CatalogReader::new(&storage, snap1).unwrap();
    ...
    let snap2 = committed_snap(&txn);
    let r2 = CatalogReader::new(&storage, snap2).unwrap();
...
fn test_delete_nonexistent_index_returns_error() {
    ...
    txn.begin().unwrap();
    {
        ...
        assert!(
            matches!(
                err,
                nexusdb_core::DbError::CatalogIndexNotFound { index_id: 9999 }
            ),
...
fn test_snapshot_before_commit_does_not_see_new_table() {
    ...
    let reader_before = CatalogReader::new(&storage, snap_before).unwrap();
    ...
    let reader_after = CatalogReader::new(&storage, snap_after).unwrap();
    assert!(reader_after
        .get_table("public", "invisible")
        .unwrap()
        .is_some());
...
fn test_snapshot_before_delete_still_sees_row() {
    ...
    let reader_old = CatalogReader::new(&storage, snap_before_delete).unwrap();
    assert!(reader_old.get_table_by_id(table_id).unwrap().is_some());

    // New snapshot does not.
    let snap_after_delete = committed_snap(&txn);
    let reader_new = CatalogReader::new(&storage, snap_after_delete).unwrap();
...
fn test_catalog_not_initialized_returns_error() {
    ...
    assert!(
        matches!(err, nexusdb_core::DbError::CatalogNotInitialized),
