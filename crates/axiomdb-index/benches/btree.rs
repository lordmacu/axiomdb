...
fn bench_point_lookup(c: &mut Criterion) {
    ...
    for size in [10_000usize, 100_000, 1_000_000] {
        ...
        group.bench_with_input(BenchmarkId::new("axiomdb_btree", size), &size, |b, &n| {
            let mut i = 0usize;
            b.iter(|| {
...
fn bench_range_scan(c: &mut Criterion) {
    ...
    for range_size in [100usize, 1_000, 10_000] {
        ...
        group.bench_with_input(
            BenchmarkId::new("axiomdb_btree", range_size),
            &range_size,
            |b, _| {
                b.iter(|| {
                    let count = tree
...
fn bench_insert_sequential(c: &mut Criterion) {
    ...
    group.bench_function("axiomdb_btree_1k", |b| {
        b.iter(|| {
            let mut tree = BTree::new(Box::new(MemoryStorage::new()), None).unwrap();
            for i in 0..1_000u64 {
                let key = format!("{:08}", i);
...
fn bench_insert_random(c: &mut Criterion) {
    ...
    group.bench_function("axiomdb_btree_100k_random", |b| {
        b.iter(|| {
            let mut tree = BTree::new(Box::new(MemoryStorage::new()), None).unwrap();
...
fn bench_insert_1m_sequential(c: &mut Criterion) {
    ...
    group.bench_function("axiomdb_btree_1m", |b| {
        b.iter(|| {
            let mut tree = BTree::new(Box::new(MemoryStorage::new()), None).unwrap();
