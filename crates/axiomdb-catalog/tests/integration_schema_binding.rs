...
fn test_resolve_table_by_default_schema() {
    ...
    let mut resolver =
        SchemaResolver::new(&storage, snap, DEFAULT_DATABASE_NAME, "public").unwrap();
...
fn test_resolve_table_not_found_error_message_is_qualified() {
    ...
    let snap = txn.snapshot();
    let mut resolver =
        SchemaResolver::new(&storage, snap, DEFAULT_DATABASE_NAME, "myschema").unwrap();
...
#[test]
fn test_database_catalog_bootstrap_includes_default_database() {
    let (storage, txn) = setup();
    let snap = txn.snapshot();
    let mut reader = CatalogReader::new(&storage, snap).unwrap();

    let databases = reader.list_databases().unwrap();
...
fn test_database_binding_separates_same_table_name_across_databases() {
    ...
    let default_table = reader
        .get_table_in_database(DEFAULT_DATABASE_NAME, "public", "events")
        .unwrap()
        .expect("default database table");
    let analytics_table = reader
        .get_table_in_database("analytics", "public", "events")
        .unwrap()
        .expect("analytics database table");

    assert_eq!(default_table.id, default_table_id);
    assert_eq!(analytics_table.id, analytics_table_id);
    assert_eq!(
        reader.effective_table_database(default_table_id).unwrap(),
        DEFAULT_DATABASE_NAME
    );
    assert_eq!(
        reader.effective_table_database(analytics_table_id).unwrap(),
        "analytics"
    );
}
...
fn test_mvcc_resolver_does_not_see_uncommitted_table() {
    ...
    let mut resolver_new =
        SchemaResolver::new(&storage, snap_after, DEFAULT_DATABASE_NAME, "public").unwrap();
...
fn test_catalog_not_initialized_returns_error() {
    ...
    let err = SchemaResolver::new(&storage, snap, DEFAULT_DATABASE_NAME, "public")
        .err()
