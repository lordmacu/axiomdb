...
### 11.25c (partial defer) — `row_to_json(row_expr)`
- [ ] `row_to_json(t)` / `row_to_json(ROW(1, 'x', true))` → JSON representation of an entire row. Requires `ROW(...)` expressions and record types as first-class values (PG extension of the type system). AxiomDB's current type system treats projections as lists of scalar `Value`s, not as a `Record` type. Add when record types land.
