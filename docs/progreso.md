...
- [ ] 11.19 ⏳ SQL/JSON standard query functions — split into sub-phases:
  ...
  - [ ] 11.19b ⏳ `PASSING` (bound jsonpath variables), `WITH WRAPPER` on JSON_QUERY, `QUOTES`/`KEEP QUOTES`/`OMIT QUOTES` clauses — each requires either a jsonpath compiler extension or a grammar-only extension; deferred until user demand prioritizes.
