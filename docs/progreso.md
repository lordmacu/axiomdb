...
- [ ] 11.22 ⏳ JSONB mutation parity — split into sub-phases:
  ...
  - [ ] 11.22b ⏳ `jsonb_set_lax` with `null_value_treatment` enum (raise_exception / use_json_null / delete_key / return_target) — PG-only, requires extra helpers for the 4-way NULL dispatch.
