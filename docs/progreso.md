...
- [ ] 11.21 ⏳ JSONPath parity + indexed path operators — split into sub-phases:
  ...
  - [ ] 11.21d ⏳ Richer JSONPath accessors (`.type()`, `.size()`, arithmetic in filter expressions), planner predicate extraction for indexable JSONPath, `jsonb_path_ops` hash-based GIN opclass (smaller/faster alternative to the current `jsonb_ops`-style term index).
