...
- [ ] 5.21 ⏳ INSERT WAL group-commit throughput — identified by local bench 50K rows: INSERT 67K r/s vs MariaDB 230K r/s (3.4×); root cause: each INSERT flushes WAL individually (fsync per row in autocommit mode) instead of batching multiple rows into one fsync; fix: WAL group-commit already exists (`CommitCoordinator`) but the benchmark runs single-row autocommit inserts — either (a) improve group-commit window to coalesce more concurrent commits, or (b) add a heap-level insert buffer that accumulates rows within a single transaction and flushes once at commit; benchmark reference: `local_bench.py --scenario insert --rows 50000` (2026-03-27)
...
- [ ] ⚠️ PERF: multi-row INSERT on indexed tables falls back to per-row maintenance — `executor/insert.rs` only uses `insert_rows_batch()` when `secondary_indexes.is_empty()`, so `INSERT ... VALUES (...),(... )` on tables with PRIMARY KEY / secondary indexes still pays heap+index maintenance row-by-row; need batch index maintenance or a PK-specialized bulk path; identified by `local_bench.py --scenario insert_multi_values --rows 5000` (2026-03-26), AxiomDB 76.6K rows/s vs MariaDB 193K / MySQL 254K
- [ ] ⚠️ PERF: indexed `UPDATE ... WHERE` path missing — `UPDATE` still materializes/scans all candidate rows before heap rewrite even when the predicate is a PK/range condition; add `plan_update_candidates` / RID collection analogous to the indexed DELETE fast path (6.3b); identified by `local_bench.py --scenario update_range --rows 5000` (2026-03-26), AxiomDB 70.2K rows/s vs MariaDB 493.9K / MySQL 316.5K
- [ ] ⚠️ PERF: SQL/wire point lookup still under target — repeated `SELECT * FROM bench_users WHERE id = literal` over the MySQL wire remains 6-8× slower than MariaDB/MySQL; verify the planner/executor path uses the PK B-Tree end-to-end without scan fallbacks and reduce per-lookup SQL/materialization overhead; identified by `local_bench.py --scenario select_pk --rows 5000` (2026-03-26), AxiomDB 1.15K lookups/s vs MariaDB 9.16K / MySQL 7.08K
