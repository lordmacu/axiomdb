...
- [ ] 6.19 ⏳ WAL fsync pipeline — server DML commits now use an always-on leader-based fsync pipeline inspired by MariaDB `group_commit_lock`: one connection becomes leader, does `flush+fsync`, queued followers piggyback if their `commit_lsn` is covered, and `Expired` followers return immediately when another leader already flushed past their LSN; timer-based `CommitCoordinator` / `group_commit_interval_ms` config removed; targeted WAL/network tests and wire smoke pass, but the key acceptance benchmark is still failing: `local_bench.py --scenario insert_autocommit --rows 1000 --table --engines axiomdb` measures only `224 ops/s`, well below the spec target of `>= 5K ops/s`
- [ ] ⚠️ 6.19 gap — the leader-based pipeline does not improve single-connection request/response MySQL workloads under the current server model because the handler still waits for durability before sending the OK packet; sequential clients cannot submit the next statement while the fsync is in flight, so the expected single-connection piggyback never materializes
...
- [ ] ⚠️ PERF: indexed UPDATE still under target after 6.17 — candidate discovery no longer full-scans PK/range/equality predicates, but `update_range` remains dominated by the apply path (heap rewrite + index maintenance) once rows are found; latest local bench after `6.17`: AxiomDB `85.2K rows/s` vs MariaDB `618K` / MySQL `291K`; next inspect stable-RID hit rate, per-row index reinsertion, and root persistence frequency
