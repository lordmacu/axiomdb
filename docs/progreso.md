...
- [ ] 5.20 ⏳ UPDATE index-skip optimization — skip delete+insert for any index whose columns are not in the SET assignments; identified by local bench 50K rows: UPDATE 50K r/s vs MariaDB 629K r/s (12×); root cause: PK index delete+insert per row even when only non-indexed columns change (`score = score + 1`); fix: before index maintenance in `executor/update.rs`, compute `changed_cols` from SET assignments, skip any index whose `IndexDef.columns` have no overlap with `changed_cols`; expected gain: 5-8× for updates that only touch non-indexed columns
