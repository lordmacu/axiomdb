...
- [ ] 4.6d ⏳ INSERT LOW_PRIORITY / HIGH_PRIORITY / DELAYED — modifiers before INTO not in lexer; `INSERT LOW_PRIORITY INTO t VALUES (1)` causes parse error; parse and discard (semantics not needed for single-server use)
- [ ] 4.5f ⏳ SELECT ... INTO @var — `SELECT name INTO @user_var FROM users LIMIT 1` not in parser or AST; common MySQL idiom for single-row variable assignment; parser/dml.rs needs an `INTO @ident` clause after the SELECT list
