...
- [ ] 11.20 🔄 `JSON_TABLE` row source — parse and execute `JSON_TABLE(...)` in `FROM` with `COLUMNS`, `FOR ORDINALITY`, `EXISTS PATH`, scalar `PATH`, and `NESTED PATH`; first target is PostgreSQL/Oracle-style shredding of JSON arrays into relational rows, later integrated into `UPDATE`, `DELETE`, and `MERGE` sources
  ...
  - [ ] 11.20b ⏳ `NESTED PATH` single-level with LEFT-OUTER NULL padding and per-level ordinality.
  - [ ] 11.20c ⏳ Multi-sibling `NESTED PATH` (UNION semantics) and multi-level nesting (recursive tree walk).
  - [ ] 11.20d ⏳ `WRAPPER` / `QUOTES` on JSON_TABLE columns; `PASSING name AS var` bindings on the row path; LATERAL-correlated `doc` expressions; `JSON_TABLE` as UPDATE/DELETE/MERGE source.
