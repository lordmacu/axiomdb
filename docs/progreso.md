...
- [ ] 11.23 ⏳ JSON Schema validation — split into sub-phases:
  ...
  - [ ] 11.23b ⏳ `JSON_SCHEMA_VALIDATION_REPORT(schema, doc)` — detailed error report with failing path + keyword + message list (currently silent bool).
  ...
  - [ ] 11.23d ⏳ Draft-07 full coverage gaps: `pattern` / `patternProperties` (regex), `format` (email/date/uuid/etc.), `oneOf`/`anyOf`/`allOf`/`not`, `$ref` internal refs, `dependencies`, `uniqueItems`, `propertyNames`, `if`/`then`/`else`.
- [ ] 11.24 ⏳ Oracle JSON compatibility surface — split into sub-phases:
  ...
  - [ ] 11.24b ⏳ `JSON_TRANSFORM` — Oracle's transformer function (multi-operation mutator with REMOVE / SET / RENAME / APPEND / INSERT / REPLACE operations in a single call).
  ...
  - [ ] 11.24d ⏳ Data Guide-style structure discovery + JSON search index hooks + document-collection APIs + JSON-relational duality view feasibility notes.
