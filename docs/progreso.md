...
- [ ] 5.2d ⏳ MySQL-compatible default collation (case-insensitive) — default to `utf8mb4_general_ci` when `CompatMode::Mysql` active; currently defaults to binary (case-sensitive)
- [ ] 5.5b ⏳ Column type wire encoding subtypes — TINYINT→0x01, SMALLINT→0x02, MEDIUMINT→0x09, YEAR→0x0d, TIME→0x0b, DATETIME→0x0c (`result.rs:465`)
...
- [ ] 6.5b ⏳ Multi-column FK constraints — `REFERENCES t (col1, col2)` → NotImplemented at 3 sites (`ddl.rs:182`, `ddl.rs:2087`)
- [ ] 6.6d ⏳ FK references not updated on RENAME TABLE — subsequent FK enforcement breaks with table-not-found after rename (`ddl.rs:1697`)
- [ ] 6.6c ⏳ ON DELETE SET DEFAULT / ON UPDATE SET DEFAULT — requires stored DEFAULT expressions in catalog (`fk_enforcement.rs:672`, `:776`)
...
- [ ] 11.18 ⏳ PostgreSQL JSONB operator parity — add key-exists and mutation/extraction operators not covered by 11.16/11.17: `?`, `?|`, `?&`, `<@`, `||`, `-`, `#-`, `#>`, `#>>`; make key-exists use the existing GIN term layout where possible; preserve NULL/error semantics in tests against PostgreSQL reference cases
- [ ] 11.19 ⏳ SQL/JSON standard query functions — implement `JSON_VALUE`, `JSON_QUERY`, and `JSON_EXISTS` with `RETURNING`, `PASSING`, strict/lax path mode, `ON EMPTY`, and `ON ERROR`; keep them separate from MySQL-style `JSON_EXTRACT` so compatibility behavior can be tested independently
...
- [ ] 11.21 ⏳ JSONPath parity + indexed path operators — add PostgreSQL-style `@?` and `@@`, `jsonb_path_match`, `jsonb_path_query_array`, JSONPath variables, richer accessors, and planner extraction for indexable JSONPath predicates; add `jsonb_path_ops` hash-based GIN opclass as a smaller/faster alternative to the current `jsonb_ops`-style term index
- [ ] 11.22 ⏳ JSONB mutation parity — implement PostgreSQL-style `jsonb_set`, `jsonb_set_lax`, `jsonb_insert`, field delete, path delete, and array-element delete semantics; document differences from MySQL `JSON_SET` / `JSON_REMOVE` and route common aliases without surprising data loss
- [ ] 11.23 ⏳ JSON Schema validation — add user-defined JSON schema validation for constraints and ad hoc checks (`JSON_SCHEMA_VALID` / `JSON_SCHEMA_VALIDATION_REPORT` or equivalent), with catalog storage for reusable schemas; this closes the Phase 11.16 deferred user-defined JSON schema validation item
- [ ] 11.24 ⏳ Oracle JSON compatibility surface — research and implement the high-value Oracle JSON features not covered by SQL/JSON standard functions: dot notation, `JSON_TRANSFORM`, `JSON_SERIALIZE`, `JSON_SCALAR`, `JSON_EQUAL`, Data Guide-style structure discovery, JSON search index full-text hooks, document-collection APIs, and JSON-relational duality view feasibility notes; split implementation specs if any item becomes too large
