...
- [ ] 5.18 ⏳ Heap insert tail-pointer cache — `HeapChain::insert` walks the full chain from root on every INSERT to find the last page (O(P) per row, P = heap pages); at 100K rows (~1000 pages) this causes 100M page reads and degrades INSERT from ~35K ops/s to ~4.6K ops/s (O(N²) total); fix: cache the last-page ID in `SessionContext` or `TableDef` and update it on chain extension; identified by AxiomDB vs MySQL bench at 100K rows (2026-03-26)
- [ ] 5.19 ⏳ B+tree batch delete (sorted single-pass) — DELETE WHERE and UPDATE do one `BTree::delete_in` per row (O(N log N) total for N rows), making DELETE WHERE 118× slower than MySQL (4.6K vs 542K ops/s at 5K rows); fix: collect all keys to delete, sort them, then do a single left-to-right O(N) pass through the tree removing all matching keys in order — same principle as Phase 6.2 bulk index build; applies equally to UPDATE (delete old key + insert new key per row); identified by AxiomDB vs MySQL bench at 5K rows (2026-03-26)
