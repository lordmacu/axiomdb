...
- [ ] 21.3b ⏳ `WITH RECURSIVE` iteration executor — extract base/step from SetOp body (parser needs to absorb UNION inside CTE paren), analyze step with stub CTE binding, implement loop: wt ← base; loop: new ← step (with cte bound to wt); dedup if !union_all; rt ← rt+new; wt ← new; stop when new empty or depth > MAX_RECURSION.
...
- [ ] 21.4b ⏳ RETURNING — INSERT + UPDATE executor wiring: capture post-write rows in heap + clustered + batch paths, project through RETURNING list, return `QueryResult::Rows`.
