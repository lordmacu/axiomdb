...
- [x] 4.18c ✅ VARCHAR(N) length validation + CHAR(N) right-padding — `type_len: u16` in catalog + AST `ColumnDef`; `is_fixed_len: bool` (flags bit3) distinguishes CHAR from VARCHAR; `enforce_text_constraints()` validates VARCHAR(N) length and right-pads CHAR(N) with spaces on INSERT/UPDATE; `DbError::DataTooLong` (MySQL 1406 / SQLSTATE 22001); 8 integration tests for padding, rejection, unicode, empty string, update
- [x] 4.18d ✅ Date string implicit coercion in comparisons — `WHERE created_at = '2024-01-01'` now implicitly parses the string as DATE/TIMESTAMP; `compare_values()` in `eval/ops.rs` adds four coercion arms for `(Timestamp,Text)/(Text,Timestamp)/(Date,Text)/(Text,Date)`; `str_to_date_inner`, `micros_to_ndt`, `days_to_ndate` promoted to `pub(crate)` in `datetime.rs`; 8 integration tests
- [x] 4.18e ✅ Default column expression persistence in catalog — `default_expr: Option<String>` in catalog ColumnDef (flags bit4, backward-compatible); `expr_to_sql` module converts AST to SQL text; CREATE TABLE + ALTER TABLE ADD/MODIFY COLUMN persist defaults; INSERT uses stored default for missing columns; `Expr::Default` in VALUES resolves to stored default; CREATE TABLE LIKE copies defaults; 9 integration tests
...
- [x] 4.25d ✅ CHECK constraints enforced on UPDATE — `check_row_constraints()` now called in `execute_update_with_candidates()` before heap writes; mirrors INSERT path
- [x] 4.7b ✅ MySQL error numbers — `axiomdb-network/src/mysql/error.rs` maps all DbError variants to correct MySQL codes+SQLSTATE: TableNotFound→1146/42S02, ColumnNotFound→1054/42S22, UniqueViolation→1062/23000, NotNullViolation→1048/23000, ForeignKeyViolation→1452/23000, ParseError→1064/42000, etc.
...
- [x] 4.9f ✅ COUNT(DISTINCT col) / SUM(DISTINCT) / AVG(DISTINCT) — DISTINCT inside aggregate functions: parser remaps to `count_distinct`/`sum_distinct`/`avg_distinct` function names; executor adds CountDistinct/SumDistinct/AvgDistinct accumulators with deduplication; MIN/MAX DISTINCT is a no-op (same as non-DISTINCT); 5 integration tests in `integration_aggregate_hash.rs`
...
- [ ] 4.10f ⏳ GROUP BY ... WITH ROLLUP — `GROUP BY dept WITH ROLLUP` generates subtotal rows with NULL group keys; not in parser or executor; fix: consume optional WITH ROLLUP after GROUP BY column list; executor adds synthetic NULL rows after each group boundary
- [x] 4.10e ✅ Positional ORDER BY / GROUP BY column ordinals — `ORDER BY 1, 2 DESC` and `GROUP BY 1` resolved via `resolve_positional_order_by()` and `resolve_positional_group_by()` in `executor/shared.rs`; `Expr::Literal(Value::Int(n))` → nth SELECT item expression (1-indexed); applied before sort/group evaluation
...
- [x] 4.12c ✅ DISTINCT with non-selected ORDER BY column — `SELECT DISTINCT a FROM t ORDER BY b` now returns ParseError (MySQL error 3065); check added after ORDER BY + SELECT resolution in `analyzer_stmt.rs`; SELECT DISTINCT * exempted (wildcard covers all); 4 integration tests
...
- [x] 4.20b ✅ SHOW CREATE TABLE — `SHOW CREATE TABLE t` reconstructs DDL from catalog (columns + types + NOT NULL + AUTO_INCREMENT + PRIMARY KEY + secondary indexes); 2-column output (`Table`, `Create Table`) matching MySQL; ENGINE=InnoDB appended; 3 integration tests
- [x] 4.22c ✅ ALTER TABLE ADD CONSTRAINT PRIMARY KEY — `ALTER TABLE t ADD PRIMARY KEY (col[, ...])` now validates existing rows for NULL/duplicates, flips PK columns to `NOT NULL`, promotes heap tables to clustered storage via the rebuild path, and preserves secondary indexes
- [x] 4.22d ✅ DROP INDEX without ON table — `DROP INDEX idx_name` scans all tables in all schemas via `find_table_for_index()` helper; first table owning an index with that name is used; IF EXISTS still works; `ddl_drop_index.rs`
- [ ] 4.11b ⏳ Subquery in JOIN — `SELECT … FROM t JOIN (SELECT …) alias ON …`; executor returns NotImplemented (`select.rs:667`, `select.rs:1141`); FROM subqueries (`FROM (SELECT …) alias`) already work (Phase 4.11); this is the JOIN variant only
- [x] 4.5c ✅ SELECT * without FROM — returns empty result set (0 columns, 0 rows) instead of NotImplemented; `SELECT 1` / `SELECT expr` without FROM already worked
- [x] 4.2d ✅ Integer literal overflow — `lexer.rs` now clamps values > `i64::MAX` to `i64::MAX` (via `u64` fallback); no more silent lex failure for large numeric literals
- [x] 4.2e ✅ Hex/binary literals — `x'48656c6c6f'` → `StringLit("Hello")`; `0b1010`/`0B1010` → `Integer(10)`; `b'1010'`/`B'1010'` → `Integer(10)`; note: `0xFF` → `HexLit` was already supported; 9 new lexer unit tests
- [x] 4.2f ✅ Double-quote strings with ANSI_QUOTES OFF — `sql_mode` now drives session-aware parsing: default `ANSI_QUOTES=OFF` treats `"..."` as string literal; `SET sql_mode='ANSI_QUOTES'` switches `"` to quoted identifiers; the same mode bit is respected by prepared statements, plan-cache normalization, and multi-statement packet splitting
- [x] 4.2b ✅ DELETE/UPDATE with ORDER BY + LIMIT — `DELETE FROM t ORDER BY col LIMIT N` and `UPDATE t SET ... ORDER BY col LIMIT N`; `order_by` + `limit` fields added to both AST nodes; parser consumes them; heap + clustered executors apply sort-then-truncate; analyzer resolves ORDER BY column refs; fast-paths (clustered full-delete, fused scan-patch) gated on order_by.is_empty() && limit.is_none(); 6 integration tests (heap+clustered, DELETE+UPDATE, with/without ORDER BY)
- [x] 4.3b ✅ LOCK/UNLOCK/FLUSH/KILL statements — `Stmt::Noop` added; LOCK/UNLOCK/FLUSH/KILL → silently succeed; lexer tokens added; executor returns `QueryResult::Empty`
- [x] 4.2c ✅ LIMIT offset, count (MySQL comma syntax) — `LIMIT 5, 10` parsed correctly; first arg = OFFSET, second = LIMIT (MySQL convention)
- [x] 4.19e ✅ CONCAT(NULL) → NULL — any NULL arg now returns NULL (MySQL-compatible); CONCAT_WS unchanged (skips NULLs by design)
- [x] 4.19f ✅ SUBSTR negative index — `SUBSTR('hello', -3)` → `'llo'`; negative index counts from end; clamped to start if abs > len
- [x] 4.19k ✅ ROUND() round-half-up — uses round-half-away-from-zero (MySQL); `ROUND(2.5)` → 3, `ROUND(-2.5)` → -3
- [x] 4.19g ✅ CONVERT(expr, type) MySQL two-argument syntax — parser handles `CONVERT(expr, SIGNED|UNSIGNED|CHAR|BINARY|DATE|DATETIME|DECIMAL|JSON)` and `CONVERT(expr USING charset)`; both desugared to Expr::Cast; parse_convert_type() handles MySQL-specific type names
- [x] 4.19h ✅ Math functions: PI, EXP, LOG (1+2-arg), LOG2, LOG10, SIN, COS, TAN, ASIN, ACOS, ATAN/ATAN2, COT, RADIANS, DEGREES, TRUNCATE, RAND, GREATEST, LEAST — all in `eval/functions/numeric.rs`; one_trig() helper for single-arg trig; compare_values() widened to pub(crate) for GREATEST/LEAST
- [x] 4.19i ✅ Date functions: UTC_DATE/TIME/TIMESTAMP, FROM_UNIXTIME (1+2-arg), UNIX_TIMESTAMP (0+1-arg), CONVERT_TZ stub, ADDDATE/DATE_ADD/SUBDATE/DATE_SUB, TIMESTAMPDIFF, MAKEDATE, DAYOFWEEK/DAYOFMONTH/DAYOFYEAR, WEEKDAY, QUARTER, WEEK/WEEKOFYEAR, YEARWEEK, LAST_DAY, DATE, TIME, TIMEDIFF — all in `eval/functions/datetime.rs`
- [x] 4.19j ✅ String functions: NVL2, INSERT (string replacement), ORD, SOUNDEX (Knuth algorithm), CHAR variadic, FORMAT, BIN, OCT, HEX, UNHEX, QUOTE, FIELD, ELT — all in `eval/functions/string.rs`; duplicate char/chr arm removed
- [x] 4.1c ✅ Column attributes — `UNSIGNED`, `ZEROFILL`, `SIGNED`, `COLLATE`, `CHARACTER SET`, `ON UPDATE CURRENT_TIMESTAMP`, `COMMENT`, `VISIBLE`/`INVISIBLE`, `STORAGE` consumed and stored/discarded in `parse_column_def`
- [x] 4.1d ✅ CREATE TABLE inline INDEX/KEY — `TableConstraint::Index` added to AST; parser handles INDEX/KEY in constraint list; executor creates non-unique indexes at CREATE TABLE time
- [x] 4.1b ✅ CREATE TABLE table options — `ENGINE=`, `CHARSET=`, `COLLATE=`, `AUTO_INCREMENT=`, `COMMENT=`, `ROW_FORMAT=`, `KEY_BLOCK_SIZE=` etc. after `)` parsed and discarded via `skip_table_options()`
- [x] 4.4b ✅ Expression operators: REGEXP/RLIKE, XOR, DIV, <=>, bitwise — `REGEXP`/`RLIKE` (regex), `XOR` (boolean XOR), `DIV` (integer division), `<=>` (null-safe eq), `&`/`|`/`^`/`~`/`<<`/`>>` (bitwise); new lexer tokens + 9 BinaryOp variants + BitNot UnaryOp; full precedence chain (parse_xor→bitor→bitand→shift→bitxor); `regex` crate for REGEXP/RLIKE eval; NullSafe bypasses NULL propagation; hex literals `0xFF` → Value::BigInt; Int/BigInt/Real→Text coercion now allowed in Strict mode
- [x] 4.4c ✅ SET SESSION/GLOBAL/LOCAL prefix + multi-variable SET — SESSION/GLOBAL/LOCAL consumed; `SET a=1, b=2` multi-var loop; `@@var`/`@var` prefix stripped; 15+ MySQL compat variables added to executor
- [x] 4.4d ✅ LIKE ESCAPE clause wired end-to-end — `escape: Option<Box<Expr>>` added to `Expr::Like`; parser stores ESCAPE char expression; evaluator calls `like_match_with_escape(text, pat, ch)` when present; NULL escape propagates as NULL; all traversal/resolution/substitution sites updated
- [x] 4.3h ✅ Standalone RENAME TABLE statement — `RENAME TABLE a TO b` and `RENAME TABLE a TO b, c TO d` (multi-pair); `RenameTableStmt` added to AST + parser + executor; checks old exists / new not in use; 2 integration tests (basic data preserved, duplicate fails)
- [x] 4.3f ✅ CREATE TABLE ... SELECT (CTAS) — `CREATE TABLE new AS SELECT * FROM src`; parser branches on AS SELECT after table name; executor runs inner SELECT, derives column types, creates heap table, bulk-inserts rows; column aliases from SELECT list become table column names; 3 integration tests (basic, column names, empty source)
- [x] 4.3g ✅ CREATE TABLE ... LIKE — `CREATE TABLE staging LIKE users`; parser reads LIKE + source table name; executor copies TableDef (columns, constraints, indexes) from catalog with a new table name; CREATE TABLE IF NOT EXISTS LIKE supported; original data not copied; 3 integration tests (heap, clustered, IF NOT EXISTS)
- [x] 4.4e ✅ INSERT IGNORE — `INSERT IGNORE INTO t VALUES (...)` silences UniqueViolation/DuplicateKey/NotNullViolation/ForeignKeyViolation per-row and continues; `ignore: bool` in InsertStmt; heap ctx: per-row error catch + undo via delete_row on index violation; clustered ctx: per-row apply loop with slice `&prepared[i..i+1]`; intra-batch PK dedup skips instead of errors; 4 integration tests (heap unique, clustered PK, no-dups all insert, INSERT IGNORE SELECT)
- [x] 4.4f ✅ SELECT FOR UPDATE / LOCK IN SHARE MODE — parsed and stored in `SelectStmt.lock_mode: Option<LockMode>`; `Token::Lock/Share/Mode` added to lexer; `FOR UPDATE` → `LockMode::ForUpdate`; `LOCK IN SHARE MODE` → `LockMode::ShareMode`; executor ignores until Phase 13.7 row-level locking
- [ ] 4.4g ⏳ Multi-table DELETE/UPDATE JOIN syntax — `DELETE t FROM t JOIN s ON ...` and `UPDATE t JOIN s ON ... SET t.col=val` not parsed (`parser/dml.rs:440-476`); MySQL-specific syntax used in data migrations and cleanup scripts; requires extending DeleteStmt/UpdateStmt with optional join clause
- [x] 4.4h ✅ CALL / DO statements — `CALL procedure(args)` → Stmt::Noop (silently succeeds); `DO expr` → Stmt::Noop; CALL/DO tokens added to lexer; top-level parser dispatches both; executor returns QueryResult::Empty; 2 integration tests
- [x] 4.4i ✅ SELECT HIGH_PRIORITY / STRAIGHT_JOIN / SQL_CALC_FOUND_ROWS modifiers — consume loop after DISTINCT in parse_select(); SQL_CALC_FOUND_ROWS stored in SelectStmt.calc_found_rows; executor calls set_found_rows(pre-limit count); FOUND_ROWS() reads thread-local; integration tests in integration_g6_functions.rs
- [x] 4.4j ✅ UPDATE SET col = DEFAULT — handled via `Expr::Default` (implemented in 4.6c); `UPDATE t SET stock = DEFAULT` now parsed and evaluated; resets to `Value::Null` pending full default persistence (4.18e)
- [x] 4.5d ✅ BEGIN WORK + START TRANSACTION READ ONLY/READ WRITE — `WORK` keyword consumed after BEGIN; `READ ONLY`/`READ WRITE` modifiers consumed and ignored (no read-only enforcement until Phase 13)
- [x] 4.20c ✅ INFORMATION_SCHEMA virtual database — 6 virtual tables: TABLES (21 cols), COLUMNS (22 cols), KEY_COLUMN_USAGE (12 cols), TABLE_CONSTRAINTS (7 cols), REFERENTIAL_CONSTRAINTS (11 cols, empty), STATISTICS (18 cols); intercepts in analyzer (`bound_table_ref`) and executor (`select_core.rs`, `select_ctx.rs`); WHERE / ORDER BY / GROUP BY / LIMIT / projection all work; 26 integration tests in `integration_g11_information_schema.rs`
- [x] 4.22e ✅ ALTER TABLE DROP/MODIFY COLUMN auto-index-drop — `DROP COLUMN` now auto-drops affected secondary indexes and remaps/rebuilds survivors as needed; `MODIFY COLUMN` rebuilds the required secondary indexes on heap and clustered tables while preserving metadata (`predicate`, `INCLUDE`, `fillfactor`, `index_type`, `pages_per_range`); PK/FK/CHECK-dependent columns still reject explicitly
- [ ] ⚠️ 4.22e follow-up — `ALTER TABLE ... ADD COLUMN` on heap tables with existing secondary indexes still needs the same survivor-rebuild treatment after RID-changing rewrites
- [x] 4.6b ✅ INSERT INTO t SET col=val syntax — MySQL-specific `INSERT SET` syntax parsed in `parser/dml.rs`; desugared to single-row VALUES with explicit column list; `INSERT INTO t SET name='Alice', score=100` fully supported
- [x] 4.6c ✅ INSERT ... VALUES (1, DEFAULT) + UPDATE SET col=DEFAULT — `Expr::Default` variant added to `expr.rs`; parser emits it for `DEFAULT` token in expression position; evaluates to `Value::Null` (executor applies column default; full default persistence in 4.18e); covers both INSERT VALUES and UPDATE assignments
- [x] 4.6d ✅ INSERT column count mismatch → error 1136 — `DbError::ColumnCountMismatch` (SQLSTATE 21S01) added; check in VALUES loop after evaluating provided values
- [x] 4.1g ✅ CREATE DATABASE IF NOT EXISTS / CHARACTER SET / COLLATE — IF NOT EXISTS handled; DEFAULT CHARACTER SET / COLLATE options consumed and discarded
- [x] 4.21b ✅ HAVING clause alias resolution — alias map built from SELECT projections; `HAVING cnt > 5` where `cnt` is a SELECT alias is substituted before evaluation; same resolution path as MySQL/PostgreSQL
- [x] 4.22h ✅ ALTER TABLE ADD INDEX / DROP INDEX / CHANGE COLUMN / AUTO_INCREMENT=N — `AddIndex`/`DropIndex`/`ChangeColumn`/`SetAutoIncrement`/`SetEngine` variants added to `AlterTableOp`; ADD INDEX delegates to `execute_create_index`; DROP INDEX frees B+tree pages + bumps schema version; CHANGE COLUMN = retype under old name then rename if names differ; AUTO_INCREMENT=N / ENGINE=name accepted as no-op; ADD KEY synonym for ADD INDEX; SHOW WARNINGS/SHOW ERRORS → Noop; 7 integration tests
- [x] 4.1e ✅ SET FOREIGN_KEY_CHECKS=0 / UNIQUE_CHECKS=0 — recognized as compat variables; suppression not wired into FK/unique paths (deferred to Phase 13)
- [x] 4.22g ✅ ALTER TABLE RENAME INDEX — `ALTER TABLE t RENAME INDEX old TO new`; `RenameIndex` variant in `AlterTableOp`; `CatalogWriter::rename_index` delete-old + insert-new; RENAME KEY synonym; no data movement; 1 integration test
- [x] 4.22i ✅ ALTER TABLE t CONVERT TO CHARACTER SET — `ConvertCharset` variant added to `AlterTableOp`; accepted as no-op (AxiomDB stores all text as UTF-8); optional COLLATE consumed; 1 integration test
- [ ] 4.22f ⏳ DROP PRIMARY KEY on clustered table — `ALTER TABLE t DROP PRIMARY KEY` returns NotImplemented (`ddl.rs:1031`); requires full table rebuild back to heap layout (reverse of `ALTER TABLE REBUILD` from 39.19); heap tables unaffected
...
- [x] 4.17e ✅ Float division by zero → NULL — `1.0 / 0` now returns NULL (MySQL-compatible); zero-check added in `eval/ops.rs` `eval_arithmetic` before `real_arith`; integer div by zero remains an error
- [x] 4.17d ✅ IS TRUE / IS FALSE / IS NOT TRUE / IS NOT FALSE predicates — `Expr::IsBoolean` variant added to AST; parser emits it from IS [NOT] TRUE/FALSE tokens; evaluator in `eval/core.rs` implements MySQL semantics (NULL IS TRUE → FALSE, NULL IS NOT TRUE → TRUE, non-zero IS TRUE → TRUE)
- [ ] 5.2d ⏳ MySQL-compatible default collation (case-insensitive) — AxiomDB defaults to `SessionCollation::Binary` (case-sensitive) but MySQL's default is `utf8mb4_general_ci` (case-insensitive); `SELECT * FROM users WHERE name = 'alice'` silently misses `'Alice'` under AxiomDB defaults; fix: when `CompatMode::Mysql` is active, default `SessionCollation` to `CaseInsensitive`; emit `collation_connection = utf8mb4_general_ci` (collation_id 45) in the server greeting
...
- [x] 4.14b ✅ LAST_INSERT_ID(expr) — 1-arg form evaluates expr, stores it as session last-insert-id, returns it; `system.rs` handles `args.is_empty()` branch (0-arg) and else branch (1-arg with `set_last_insert_id`)
- [x] 4.5e ✅ SQL_CALC_FOUND_ROWS / FOUND_ROWS() — parser consumes `SQL_CALC_FOUND_ROWS` modifier into `SelectStmt.calc_found_rows`; executor calls `set_found_rows(rows.len())` before applying LIMIT; evaluator reads via `found_rows_value()`
- [x] 4.5f ✅ AUTO_INCREMENT=0 → next sequence value — `INSERT ... VALUES (0)` on AUTO_INCREMENT column now assigns next id (same as NULL); both `assign_auto_increment` and ctx path fixed
- [x] 5.11d ✅ COM_CHANGE_USER (0x11) — resets session state and returns OK (mirrors COM_RESET_CONNECTION); JDBC connection pool recycling now works; `handler.rs` adds `0x11 =>` arm
- [x] 5.10b ✅ Prepared statement param count guard — `parse_execute_packet()` rejects `param_count > 65535` before null-bitmap allocation; prevents large alloc on forged packets
- [x] 5.4c ✅ COM_STATISTICS (0x09) stub — returns `"Uptime: N  Threads: N  Questions: N ..."` string via `send_execute_packet`; monitoring agents no longer get error 1047
- [x] 5.4b ✅ COM_* unhandled commands graceful error — explicit match arms for 9 deprecated/unsupported COM_* commands (FIELD_LIST, CREATE_DB, DROP_DB, REFRESH, SHUTDOWN, PROCESS_INFO, DEBUG, BINLOG_DUMP, STMT_FETCH) return ERR 1235 (ER_NOT_SUPPORTED_YET) with specific command name; connection stays alive (was already surviving unknown commands via 1047 fallback, now with better diagnostics)
- [x] 5.9i ✅ Session variables return None for common ORM capability queries — fixed via 5.9j/5.9k/5.9l
- [x] 5.9j ✅ Missing session variables — `character_set_system`→"utf8mb3", `init_connect`→"", `collation_server`→(already present); all with `self.variables` override priority
- [x] 5.9k ✅ `@@global.var` / `SET @@global.var` — `get_variable()` strips `@@global.` prefix; `apply_set()` strips `@@global.` prefix; `SELECT @@global.max_allowed_packet` now resolves correctly
- [x] 5.9l ✅ Additional capability variables — `performance_schema`, `have_query_cache`, `log_bin`, `net_buffer_length`, `transaction_read_only`, `wait_timeout`, `interactive_timeout`, `net_write_timeout/read_timeout`, `connect_timeout`, `max_connections`, `skip_networking`, `have_openssl`, `license`, `protocol_version`, `event_scheduler`, `default_storage_engine`, `innodb_version`, `system_time_zone`; all with SET-override priority; 256 network tests pass
- [ ] 5.5b ⏳ Column type wire encoding missing integer subtypes and date subtypes — `datatype_to_mysql_type()` (`result.rs:465`) does not map TINYINT→0x01, SMALLINT→0x02, MEDIUMINT→0x09, YEAR→0x0d, TIME→0x0b, DATETIME→0x0c correctly; wrong type codes cause MySQL Workbench and JDBC to display values incorrectly; fix: add ColumnType variants and map them
- [x] 5.9f ✅ SHOW FULL TABLES / SHOW FULL COLUMNS / SHOW TABLE STATUS — `SHOW FULL TABLES` adds `Table_type="BASE TABLE"` 2nd column; `SHOW FULL COLUMNS FROM t` adds Collation/Privileges/Comment (9 cols total); `SHOW TABLE STATUS [FROM schema] [LIKE pat]` returns 18-column result with row count from stats, InnoDB engine, utf8mb4_general_ci collation; AST: `full: bool` in ShowTablesStmt/ShowColumnsStmt, new ShowTableStatusStmt; parser: FULL modifier, TABLE STATUS, FROM/IN schema, LIKE pattern; executor: ddl_show.rs updated, dispatch_ctx handles with db context; 10 integration tests
- [x] 5.9g ✅ SHOW ENGINES / SHOW CHARSET / SHOW COLLATION — parser + AST variants + static executor; SHOW ENGINES returns InnoDB DEFAULT with Transactions=YES; SHOW CHARSET returns utf8mb4/utf8/latin1/binary rows; SHOW COLLATION returns 5 standard collations; all dispatch through dispatch_ctx before the other fallthrough; 3 integration tests
- [x] 5.9e ✅ SHOW WARNINGS / SHOW ERRORS — `Stmt::ShowWarnings { limit: Option<u64> }` and `Stmt::ShowErrors { limit: Option<u64> }` AST variants; parser emits these instead of Noop, with optional `LIMIT N` parsing; `show_warnings_result(warnings: &[SqlWarning]) -> QueryResult` helper (Level/Code/Message 3-column result set) added to executor; `dispatch_ctx` reads `ctx.warnings` for ShowWarnings, filters to level=="Error" for ShowErrors; LIMIT truncates result; fallback `dispatch()` returns empty 3-column result for sessions without context; 3 integration tests
...
- [ ] 6.5b ⏳ Multi-column FK constraints — `REFERENCES t (col1, col2)` returns `NotImplemented` at 3 sites in `ddl.rs` (`ddl.rs:182` ADD CONSTRAINT, `ddl.rs:2087` DROP CONSTRAINT); requires encoding compound FK keys in `axiom_foreign_keys`; used by schemas with composite natural keys (e.g., `(country, tax_id)` → parent)
- [ ] 6.6d ⏳ FK references not updated on RENAME TABLE — `ALTER TABLE users RENAME TO accounts` does not update FK catalog entries that reference the old table name (`ddl.rs:1697`); subsequent FK enforcement breaks with table-not-found; fix: after renaming, scan axiom_foreign_keys and update all parent/child references pointing to the old table
- [x] 6.6b ✅ ON UPDATE CASCADE / ON UPDATE SET NULL — `apply_fk_update_children()` mirrors ON DELETE pattern; CASCADE propagates new parent key to child FK column; SET NULL nullifies child FK column; handles both heap and clustered child tables with full secondary index maintenance; 6 integration tests
- [ ] 6.6c ⏳ ON DELETE SET DEFAULT / ON UPDATE SET DEFAULT — FK reference actions that set the child column to its DEFAULT value when the parent is deleted/updated; `fk_enforcement.rs:672` and `:776`; requires stored DEFAULT expressions in catalog (currently not persisted); blocks schemas that use `SET DEFAULT` referential actions
...
- [ ] 11.3 ⏳ In-memory mode — `open(":memory:")` without disk; `MemoryStorage` already exists and is used throughout the test suite, but the embedded `Db::open()` does not detect `":memory:"` and route to `MemoryStorage`; adding `Db::open_memory()` or `:memory:` path detection in `embedded/src/lib.rs` would close this
...
- [x] ✅ 39.7 clustered dead-cell purge resolved in `39.18` clustered `VACUUM`
...
- [ ] ⚠️ 39.8 relocate-update still rewrites only the current inline version; older-version reconstruction remains deferred — revisit in later clustered MVCC / version-chain work
...
- [ ] ⚠️ 39.9 clustered PK bookmarks now back clustered SQL `INSERT` and `SELECT`, but FK enforcement and index-integrity rebuild flows still use RecordId-based secondary indexes — revisit in later clustered executor / maintenance work
...
- [x] ✅ 39.10 clustered overflow-chain cleanup resolved in `39.18` clustered `VACUUM`
...
- [x] ⚠️ 39.13 standalone clustered `CREATE INDEX` — resolved in Phase 40.1b: `ensure_heap_runtime` guard removed, clustered branch uses `scan_clustered_table` + `ClusteredSecondaryLayout::derive`; partial predicates, NULL skip, uniqueness check, bloom filter, stats all work; 9 integration tests
- [x] 39.14 ✅ Executor integration: INSERT into clustered table — ctx/non-ctx executor paths now write explicit-PK tables directly through `clustered_tree`, derive PK bytes from clustered primary-index metadata, bootstrap clustered `AUTO_INCREMENT` from clustered rows, maintain non-primary indexes through clustered PK bookmarks, persist table/index root changes, flush pending heap batches before the clustered statement boundary, and keep rollback/savepoint safety through clustered WAL plus index undo
- [ ] ⚠️ 39.14 reusing a delete-marked clustered PK rewrites only the current physical version; older-snapshot reconstruction of the superseded tombstone remains deferred — revisit in later clustered MVCC / version-chain work
- [x] 39.15 ✅ Executor integration: SELECT from clustered table — ctx/non-ctx SELECT paths now route clustered full scans, PK point lookups, PK range scans, and secondary-index bookmark probes through the clustered tree; clustered covering plans are normalized away from heap-era `IndexOnlyScan`, and clustered reads now preserve MVCC visibility at the clustered row header
- [ ] ⚠️ 39.15 clustered covering reads still degrade to clustered row fetches instead of a true clustered index-only path — revisit in later clustered scan optimization work
- [x] 39.16 ✅ Executor integration: UPDATE on clustered table — ctx/non-ctx UPDATE now routes explicit-PK tables through clustered candidate discovery (full scan, PK lookup/range, and secondary bookmark probes), records exact clustered row images in WAL for in-place / relocate / PK-change updates, rewrites clustered secondary bookmark indexes with undo-safe delete+insert tracking, and restores both base rows and secondary bookmarks correctly on ROLLBACK / SAVEPOINT undo
- [x] 39.17 ✅ Executor integration: DELETE from clustered table — ctx/non-ctx DELETE now routes explicit-PK tables through clustered candidate discovery (full scan, PK lookup/range, and secondary bookmark probes), enforces parent-side FK restrictions before the first delete-mark, records exact clustered row images in WAL for rollback/savepoint safety, and leaves clustered secondary bookmarks in place for deferred cleanup
- [ ] ⚠️ 39.17 FK enforcement still rejects clustered child tables — revisit in later clustered FK work
- [x] 39.18 ✅ VACUUM for clustered index — clustered `VACUUM` now physically removes safe delete-marked leaf cells, frees overflow chains, conditionally defragments fragmented clustered leaves, cleans secondary bookmark entries by clustered physical existence after purge, persists any CoW secondary-root rotation back to the catalog, and keeps uncommitted clustered deletes untouched
- [x] 39.19 ✅ Table rebuild: heap to clustered migration — `ALTER TABLE ... REBUILD` now migrates legacy heap+PRIMARY KEY tables into clustered storage via PK-ordered scan, clustered-root build, secondary bookmark rebuild, storage flush before metadata swap, and deferred free of old heap/index pages after commit
...
- [ ] ⚠️ 39.19b ALTER TABLE ADD/DROP COLUMN on clustered tables — `rewrite_rows_clustered()` in `executor/ddl.rs` is implemented and staged (246 lines, `clustered_tree::range` + `update_with_relocation` + WAL undo safety; 13 tests in `integration_clustered_alter_table.rs`) but NOT YET COMMITTED — close this subfase to record it
- [x] 39.20 ✅ Integration tests and benchmarks for clustered tables — 105 integration tests across 11 files: `integration_clustered_{insert,select,update,delete,vacuum,rebuild,secondary,insert_batch,create_index,create_table,alter_table}`.rs; criterion benchmarks: `bench_clustered_insert` (ClusteredInsertBatch at 55.9K r/s), `bench_clustered_update` (point UPDATE via PK), `group_by_age_avg_score_clustered_50k` (GROUP BY + AVG on 50K-row clustered table)
...
- [x] 40.2 ✅ Statement plan cache — OID-based invalidation: `TableDef::schema_version` bumped by DDL; `PlanDeps` AST walker tracks `(table_id, schema_version)` per query; two-level staleness check (O(1) fast path when no DDL, O(t) catalog scan only when DDL happened); `CachedPlanSource` with `generation`, LRU eviction, metrics; `PreparedStatement` migrated from global `compiled_at_version` to `deps: PlanDeps`; belt-and-suspenders: eager `invalidate_table()` after DDL + lazy `is_stale()` at lookup; DDL on `users` no longer evicts plans for `orders`, `products`, or other tables
- [x] 40.3 ✅ StorageEngine interior mutability — all `&mut self` methods changed to `&self`; `PageLockTable` (64-shard per-page RwLock, InnoDB-inspired); `MmapStorage` refactored with `Mutex<FreeList>`, `AtomicBool`, `AtomicU64`, `grow_lock: Mutex<()>`; `MemoryStorage` refactored with `RwLock<MemoryStorageInner>`; ~190 call sites updated across all crates; trait now `Send + Sync`; pre-existing bug fixed: clustered secondary index stale entries on re-insert after MVCC delete
- [x] 40.3b ✅ ALTER TABLE ADD/DROP/MODIFY COLUMN for clustered tables — `rewrite_rows` dispatches to `rewrite_rows_clustered` (range scan + `update_with_relocation` + WAL); `alter_drop_column` rejects indexed columns; `alter_modify_column` with strict type coercion + secondary-index guard + PK nullability guard + catalog delete+recreate; `coerce.rs` Int/BigInt/Real/Bool→Text; MODIFY COLUMN parser fully implemented; 13 integration tests
- [x] 40.3c ✅ INSERT DEFAULT VALUES + SHOW INDEX — `InsertSource::DefaultValues` implemented at all 4 call sites (heap+ctx, heap, clustered+ctx, clustered): builds all-NULL row then applies AUTO_INCREMENT; `SHOW INDEX FROM t` / `SHOW INDEXES` / `SHOW KEYS` parse + execute: 14-column MySQL-compatible result set, PRIMARY as Key_name="PRIMARY", Non_unique=0/1; `ShowIndexStmt` AST node; 9 integration tests; `docs/gaps-mysql-compat.md` gap inventory created
- [x] 40.4 ✅ Concurrent WAL Writer — `ConcurrentWalWriter` with lock-free LSN reservation (`AtomicU64::fetch_add` ~2 ns), `Mutex<WriteQueue>` for entry submission, `Mutex<WriterState>` for disk I/O, group commit leader/follower; `Drop` impl flushes queue to OS buffer; `TxnManager` and `Checkpointer` updated to use `ConcurrentWalWriter`; 10 unit tests (including 8-thread × 125 = 1000 concurrent appends); criterion benchmarks
- [x] 40.4b ✅ Per-connection TxnState — `ActiveTxn` removed; `ConnectionTxn` public with txn_id, undo_ops, savepoints, clustered_roots, wal_scratch, deferred_commit, active_ids_at_begin; `TxnManager` lightweight coordinator with `AtomicU64` max_committed, `RwLock<HashSet<TxnId>>` active_set, `AtomicU64` lowest_active_id (DuckDB GC horizon); `begin()` returns `ConnectionTxn`, `commit(conn)` / `rollback(conn, storage)` take ownership; atomic commit: advance max_committed + remove from active_set under same write lock (PostgreSQL ProcArrayLock pattern); atomic snapshot: read max_committed + clone active_set under same read lock; `TransactionSnapshot.active_ids: Arc<HashSet<TxnId>>` for correct multi-writer MVCC visibility; `ExecutionContext<'a>` bundles storage+coord+bloom (one-time signature sweep, 40.11 adds lock_mgr); `active_snapshot(conn_txn)` handles RC (fresh) vs RR (frozen); `SessionContext.conn_txn: Option<ConnectionTxn>` per-connection; 25 WAL unit tests + 5 wire protocol assertions (autocommit, explicit txn, rollback, savepoint, final state)
- [x] 40.5 ✅ Lock Manager — new `axiomdb-lock` crate; 5 lock modes (IS, IX, S, X, AutoIncrement) with const 5×5 conflict matrix (InnoDB `lock_compatibility_matrix` pattern); `LockFlags` bitfield (WAITING, GAP, REC_NOT_GAP, INSERT_INTENTION, TABLE_LOCK); `SlotBitmap` per-page record bitmap (InnoDB `lock_rec_t` pattern, Vec<u64> grows on demand); `LockQueue` with granted Vec + FIFO `VecDeque<LockWaiter>` (PostgreSQL `waitProcs` pattern); `LockManager` with 64 `Mutex<HashMap>` shards for records + 16 for tables; `tokio::sync::Notify` async wait; lock upgrade S→X in-place; same-txn bitmap extension; `release_all_locks(txn_id, held)` with bulk waiter promotion; `abort_waiter()` hook for 40.6 deadlock detector; `TxnLockTracker` per-txn lock ref tracking; 31 tests (conflict matrix 25 cells, S+S compatible, S+X wait, X+X wait, FIFO ordering, table IS/IX, upgrade, bitmap extend, bulk release, timeout, stress 8×1000 disjoint + 4×50 contended)
- [x] 40.6 ✅ Deadlock Detection — Brent's cycle-finding algorithm (InnoDB `Deadlock::find_cycle` pattern, O(n) time, O(1) space); synchronous on every lock wait (not PostgreSQL's timeout-based); `WaitForMap: Mutex<HashMap<TxnId, WaitEdge>>` separate from shard locks (InnoDB `wait_mutex` pattern); `detect_cycle()` returns cycle members; `select_victim()` score-based: fewer undo ops + newest txn + least wait time; victim aborted via existing `abort_waiter()` hook; wait_for entries cleaned up on grant/timeout/abort; integrated into `acquire_record_lock`; 7 deadlock unit tests + 31 lock manager tests = 38 total passing
- [x] 40.7 ✅ HeapChain Concurrent Access — page X-latch integrated into all heap mutations: `insert()`, `insert_with_hint()`, `delete()`, `delete_batch()` acquire `PageLockTable.write(page_id)` before read-modify-write (PostgreSQL `LockBuffer(BUFFER_LOCK_EXCLUSIVE)` pattern); atomic chain growth with latch ordering (new_page before last_page, prevents deadlock); `HeapInsertHint` (AtomicU64 last_page + AtomicU32 estimated_free) for O(1) tail-page suggestion across transactions; `page_lock_table()` method added to `StorageEngine` trait + implemented for MmapStorage, MemoryStorage, CountingStorage, CountingPrefetchStorage; readers unchanged (MVCC visibility, no latch needed); 48 heap tests passing
- [x] 40.8 ✅ B-tree Latch Coupling — full hybrid optimistic/pessimistic latch-coupling protocol for both the index B-tree (`crates/axiomdb-index/src/tree_{insert,delete,range}.rs`) and the clustered B-tree (`crates/axiomdb-storage/src/clustered_tree/{btree_insert,delete,search,iter}.rs`); read-path S-latch coupling with no-wait child latches and restart on contention; optimistic write fast path X-latches the leaf only; pessimistic write descent now drops the parent X-latch on safe descent (`child_is_safe_for_insert` / `child_is_safe_for_delete`) so concurrent readers can resume on the cleared internal pages; 8-thread × 10K stress test (`test_concurrent_insert_in_eight_threads_all_keys_reachable`) plus reader/writer stress on a pre-populated tree (`test_concurrent_readers_during_inserts_no_lost_keys`); spec: `specs/fase-40/spec-40.8-btree-latch-coupling.md`
  - [ ] ⚠️ Concurrent root publication — requires metapage versioning (PostgreSQL `_bt_insert_parent` pattern) or B-link tree. InnoDB holds index->lock X-latch during root splits (same serialization as our tree guard). Architectural decision: keep tree guard for now — correct, proven by InnoDB, acceptable for most workloads. Future: metapage versioning (~500-1000 LOC)
  - [ ] ⚠️ Internal-children early X-latch release for delete — blocked by CoW: internal rebalance allocates fresh page IDs, so parent child pointers change. InnoDB avoids this by using in-place page modification under buffer pool latches. AxiomDB's CoW model requires holding parent latch during internal rebalance. Architectural decision: leaf-only early release is sufficient — internal nodes are rare in the hot path
- [x] 40.9 ✅ FreeList Tier-1 — `LocalPageBatch` in `axiomdb-storage`: per-connection batch of 64 pages (adaptive up to 256) from global `Mutex<FreeList>`; `pop_or_refill` O(1) fast path with zero locks; `batch_alloc_page` canonical entry point for hot paths; `alloc_page_batch`/`free_page_batch` trait methods with default impls; `MmapStorage::recycle_queue: crossbeam_queue::SegQueue<u64>` lock-free recycle; `extension_waiters: AtomicU32` adaptive sizing; `FreeList::alloc_batch_sequential` contiguous-run scan; commit drains freed→recycle_queue + available→bitmap (steal protection); rollback drains available→bitmap; savepoint truncates freed; wired through all hot paths: heap_chain_write, clustered_tree insert/overflow, index tree insert/delete; bench: 28.8× single-thread, 703× 8-thread vs per-page Mutex baseline
- [x] 40.10 ✅ Database Lock Redesign — `Arc<RwLock<Database>>` eliminated; `SharedDatabase` struct with per-subsystem synchronization: `MmapStorage` interior mutability (40.3), `TxnManager` atomic coordinator (40.1/40.4b), `BloomRegistry`, `FsyncPipeline`, `catalog_lock: tokio::sync::RwLock<()>` (DML=read, DDL=write); `ConnectionEngineCtx` bundles `SessionContext`+`SchemaCache`+`PlanCache` per connection; handler.rs refactored from 3 loose locals to single engine context with `sync_from_wire()`/`sync_database_to_wire()`/`reset()`; clippy needless-mut cleanup (~20 callsites across 11 files); DML fully concurrent (catalog read lock only), DDL serialized (catalog write lock); schema invalidation via atomic `schema_version` bump; spec: `specs/fase-40/spec-40.10-database-lock-redesign.md`
- [x] 40.11 ✅ Executor Refactoring — dual sync/async lock API: `SyncNotify` (Condvar-based) + `WaiterSignal` enum (Async/Sync dispatch); `acquire_record_lock_sync()` + `acquire_table_lock_sync()` with identical grant/conflict logic; `ExecutionContext.lock_mgr: Option<&LockManager>` field + `execute_with_ctx_locked()` entry point; IX(table) in INSERT/UPDATE/DELETE entry points (InnoDB `INS_NODE_SET_IX_LOCK` pattern); X(row) per candidate in UPDATE (`update_candidates.rs`) and DELETE (`delete.rs`) BEFORE modification (InnoDB `lock_clust_rec_modify_check_and_lock` pattern); re-verification after `WaitGranted` via `reread_heap_row_if_visible()` (InnoDB pessimistic re-check / PostgreSQL EvalPlanQual); `release_all_for_txn()` brute-force shard scan on COMMIT/ROLLBACK; `SharedDatabase.lock_mgr: LockManager` wired through all DML paths; 8 sync unit tests + mixed async/sync waiter test; spec: `specs/fase-40/spec-40.11-executor-refactoring.md`
- [x] 40.12 ✅ Integration Tests & Benchmarks — 8 new integration tests: concurrent INSERT different rows, concurrent UPDATE different rows, MVCC repeatable read (snapshot isolation within explicit txn), DELETE invisible during txn (MVCC), autocommit stress (4 clients × 50 INSERTs), rollback releases locks (Client A rollback → Client B succeeds), mixed workload stress (4 threads × 100 ops: INSERT/UPDATE/SELECT/DELETE), lock contention stress (4 threads × 50 updates on same 10 rows); total 2540 workspace tests passing; spec: `specs/fase-40/spec-40.12-integration-tests.md`
  - [x] ⚠️ Wire protocol concurrent bench script — `benches/comparison/concurrent_bench.py`: 4 scenarios (insert, update_random, update_hotspot, mixed), multi-client threading, auto server lifecycle, percentile latency reporting, scaling summary. Verified: 1/2/4 clients × insert/update/mixed all complete with 0 errors (debug build)
  - [x] ⚠️ Crash recovery under concurrency (Test 15) — `tools/crash-recovery-test.py`: 4 concurrent writers → SIGKILL → restart → verify. 10 assertions: row count valid, sample structure, per-worker spot check, post-recovery INSERT/UPDATE/DELETE. All passing
