...
- [ ] 11.21 ⏳ JSONPath parity + indexed path operators — split into sub-phases:
  ...
  - [ ] 11.21b ⏳ `@?` and `@@` binary operators (SQL operator surface for `jsonb_path_exists` / `jsonb_path_match`) — requires lexer work (`@?` new token; `@@` currently reserved as MySQL session-var prefix, needs contextual disambiguation).
  - [ ] 11.21c ⏳ JSONPath variables (`$var`) + `PASSING` bindings + richer accessors (`.type()`, `.size()`, arithmetic in filters); planner predicate extraction for indexable JSONPath; `jsonb_path_ops` hash-based GIN opclass.
