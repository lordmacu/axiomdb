...
>
> **Core DB progress: 130/479 subphases (27.1%) — Phase 11 active**
>
> **Wishlist / feature extras** (Vector/GIS, GraphQL, OData, Toolkit, MongoDB/DoltDB/Arrow, AI, Distributed, AxiomQL, Browser Wasm) moved to [`features-roadmap.md`](./features-roadmap.md) — 264 items beyond core DB scope.
...
- [ ] 11.18 ⏳ PostgreSQL JSONB operator parity — split into sub-phases:
  ...
  - [ ] 11.18c ⏳ Remaining PG JSONB operators: `-(text[])`, `#-`, `#>`, `#>>` — same `TEXT[]` dependency as 11.18b. Either introduce native `TEXT[]` SQL type or extend each to accept JSONB-array path with documented divergence.
...
- [ ] 11.25 ⏳ JSON SRF + aggregates + construction helpers (PG paridad "streaming") — cerrar la brecha identificada tras 11.24: tras completar 11.18c/11.20d4/11.21h/11.23c/11.24c tenemos paridad sobre storage/query/indexing pero falta el *unnesting/construction* idiomatico de PG usado en producciones reales. Split:
  - [ ] 11.25a ⏳ **SRF (set-returning functions)**: `jsonb_each(doc)`, `jsonb_each_text(doc)`, `jsonb_object_keys(doc)`, `jsonb_array_elements(doc)`, `jsonb_array_elements_text(doc)`. Semantica: expanden un doc JSONB a filas (key,value) / (key) / (value). Integracion con FROM como row source — puede reusar la infra `FromClause::JsonTable` existente via shim (e.g. `jsonb_each(doc)` ≡ `JSON_TABLE(doc, '$.*' COLUMNS (key TEXT FOR KEY, value JSONB PATH '$'))`) o implementarlos como variantes nativas `FromClause::JsonbEach{..}` / `JsonbArrayElements{..}`. Primera opcion mas barata; segunda mas clara en EXPLAIN. MySQL no tiene equivalente directo.
  - [ ] 11.25b ⏳ **Aggregates**: `jsonb_agg(expr)`, `json_agg(expr)` — concatena valores en array JSONB/JSON; `jsonb_object_agg(key_expr, value_expr)`, `json_object_agg(...)` — construye objeto desde pares. Ordenable via `ORDER BY` dentro del aggregate. MySQL: `JSON_ARRAYAGG` y `JSON_OBJECTAGG` cubren equivalente. Ambos parseables como la misma funcion con nombre alternativo.
  - [ ] 11.25c ⏳ **Construction helpers**: `jsonb_build_object(k1, v1, ...)`, `jsonb_build_array(v1, v2, ...)`, `to_jsonb(anyvalue)` — mappean tipos SQL a JSONB scalar/array/object. `row_to_json(row_expr)` — proyecta fila completa a JSONB object; requiere que el type checker maneje `ROW(...)` como valor agregado (ya parcialmente soportado por `SELECT *` proyeccion).
