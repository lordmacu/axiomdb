...
- [ ] 5.9i ⏳ Session variables return None for common ORM capability queries — `performance_schema`, `have_query_cache`, `log_bin`, `net_buffer_length`, `transaction_read_only`, `sql_mode` all return None from `get_variable()` (`session.rs:584`); ORMs query these on connect; fix: add safe defaults before the fallthrough
