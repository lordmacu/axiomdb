...
- [ ] 4.18c ⏳ VARCHAR(N) length validation + CHAR(n) right-padding — `VARCHAR(5)` currently stores strings of any length without truncation or error (`executor/insert.rs`); `CHAR(5)` is treated identically to VARCHAR with no right-padding; fix: (1) validate varchar len on INSERT/UPDATE, truncate with warning in permissive mode, return DataTooLong in strict mode; (2) add `ColumnType::Char(u32)` or a padding flag, right-pad on write, strip on comparison
- [ ] 4.18d ⏳ Date string implicit coercion in comparisons — `WHERE created_at = '2024-01-01'` does not implicitly parse the string as DATE/TIMESTAMP when the column is a TIMESTAMP; MySQL coerces it automatically; fix: in type coercion / eval, when comparing Text against Timestamp/Date column, attempt STR_TO_DATE parsing before returning a type mismatch error
- [ ] 4.18e ⏳ Default column expression persistence in catalog — `ColumnDef` in `schema.rs:503` has no `default_value` field; DEFAULT expressions are parsed but silently discarded; affects `INSERT DEFAULT VALUES` fallback, `INSERT ... VALUES (1, DEFAULT)`, FK `ON DELETE/UPDATE SET DEFAULT`, and `ALTER TABLE ALTER COLUMN SET/DROP DEFAULT`; fix: add `default_expr: Option<String>` (SQL text) to persisted ColumnDef; serialize/deserialize in catalog
...
- [ ] 4.25d ⏳ CHECK constraints not evaluated on UPDATE — `check_row_constraints()` is called on INSERT but never on UPDATE (`executor/update.rs`); any CHECK (price > 0) can be violated via UPDATE; fix: add check_row_constraints() call in execute_update_with_candidates() before writing rows
- [ ] 4.7b ⏳ MySQL error numbers not returned in ERR packets — AxiomDB uses PostgreSQL SQLSTATE codes (42P01 for table not found, 42703 for column not found, 23502 for NOT NULL) instead of MySQL codes (42S02, 42S22, 23000) and wrong MySQL error numbers; JDBC/ORMs match on error number first; fix: add mysql_error_code() to DbError mapping 1146/1054/1048/1062 etc.
...
- [ ] 4.9f ⏳ COUNT(DISTINCT col) / SUM(DISTINCT) / AVG(DISTINCT) — DISTINCT inside aggregate functions not parsed (`parser/expr.rs:422`); only GROUP_CONCAT supports it; fix: in parse_ident_or_call, consume optional DISTINCT after `(`; executor deduplicates values before accumulating when distinct=true
...
- [ ] 4.10e ⏳ Positional ORDER BY / GROUP BY column ordinals — `ORDER BY 1, 2 DESC` and `GROUP BY 1` (column position in SELECT list) are not recognized; MySQL and SQL standard both support this; every ORM-generated pagination query uses it; fix: resolve integer literals in ORDER BY/GROUP BY to corresponding SELECT projection positions before evaluation
...
- [ ] 4.12c ⏳ DISTINCT with non-selected ORDER BY column — `SELECT DISTINCT a FROM t ORDER BY b` (where b is not in the SELECT list) silently executes with undefined ordering; MySQL returns error 3065; fix: in semantic analyzer, verify every ORDER BY expression is either in the DISTINCT SELECT list or derived from it
...
- [ ] 4.20b ⏳ SHOW CREATE TABLE — `SHOW CREATE TABLE t` reconstructs the full `CREATE TABLE` statement from catalog (columns + types + constraints + indexes); required by `mysqldump`, MySQL Workbench, Sequel Pro, and schema migration tools
- [ ] 4.22c ⏳ ALTER TABLE ADD CONSTRAINT PRIMARY KEY — `ALTER TABLE t ADD PRIMARY KEY (col)`; currently returns NotImplemented (`ddl.rs:2263`); requires rewriting the table to clustered storage (same path as `ALTER TABLE REBUILD`); blocks ORMs that add PK after table creation
- [ ] 4.22d ⏳ DROP INDEX without ON table — `DROP INDEX idx_name` without `ON table_name`; some MySQL clients omit the table; executor returns NotImplemented (`ddl.rs:1000`); fix: look up index name across all tables in the schema
...
- [ ] 4.5c ⏳ SELECT * wildcard without FROM — `SELECT *` without a FROM clause returns NotImplemented (`select.rs:783`); fix: return an empty row when no table is referenced and wildcard is requested; `SELECT 1` / `SELECT NOW()` already work (Phase 4.5a)
- [ ] 4.2d ⏳ Integer literal overflow silently dropped — literals exceeding i64::MAX (e.g. `9999999999999999999`) cause silent parse failure; lexer uses `.parse::<i64>().ok()` swallowing overflow; fix: try i64 first, fall back to u64, then f64 with precision warning
- [ ] 4.2e ⏳ Hex/binary literals not parsed — `0xFF`, `x'48656c6c6f'`, `0b1010`, `b'1010'` not in lexer; common in MySQL migrations and bitmask queries; add regex patterns: 0x hex → integer, x'...' → bytes, 0b/b'...' → integer
- [ ] 4.2f ⏳ Double-quote strings with ANSI_QUOTES OFF — MySQL default is ANSI_QUOTES=OFF so `"value"` is a string literal; AxiomDB always parses `"..."` as DqIdent (identifier); add `ansi_quotes: bool` to SessionContext; toggle via `SET sql_mode='ANSI_QUOTES'`
- [ ] 4.2b ⏳ DELETE/UPDATE with ORDER BY + LIMIT — `DELETE FROM t ORDER BY col LIMIT N` and `UPDATE t SET ... ORDER BY col LIMIT N` are not in `DeleteStmt`/`UpdateStmt` AST structs (`ast.rs:264`); common for safe batch processing (delete oldest N rows, update lowest-priority N records); fix: add `order_by` + `limit` fields to both AST nodes and parse them
- [ ] 4.3b ⏳ LOCK/UNLOCK/FLUSH/KILL statements — not in lexer; `mysqldump` sends all four; short-term: parse and silently ignore LOCK/UNLOCK/FLUSH for import compatibility; KILL QUERY/CONNECTION should interrupt the target session
- [ ] 4.2c ⏳ LIMIT offset, count (MySQL comma syntax) — `LIMIT 5, 10` means skip 5 return 10 but is parsed as `LIMIT 5` ignoring the `, 10`; first number is OFFSET, second is COUNT — reversed from standard `LIMIT count OFFSET offset`; extremely common in legacy MySQL code
- [ ] 4.19e ⏳ CONCAT(NULL) bug — `CONCAT(NULL, 'a')` returns `'a'` instead of `NULL` (string.rs:200); comment wrongly says "MySQL behavior" but MySQL CONCAT returns NULL on any NULL arg; it is CONCAT_WS that skips NULLs; one-line fix: change `Value::Null => {}` to `return Ok(Value::Null)` in the concat arm
- [ ] 4.19f ⏳ SUBSTR negative index bug — `SUBSTR('hello', -3)` returns `''` instead of `'llo'` (string.rs:154); negative `Int` cast to `usize` wraps to huge number, clamps to `chars.len()`, returns empty; fix: detect `n < 0` and compute `chars.len().saturating_sub(n.unsigned_abs())`
- [ ] 4.19k ⏳ ROUND() uses banker's rounding instead of MySQL round-half-up — `ROUND(2.5)` returns 2 (IEEE 754) but MySQL returns 3; `numeric.rs:90` uses Rust `.round()`; fix: use `(f * factor + 0.5 * sign).floor() / factor` for round-half-up semantics
- [ ] 4.19g ⏳ CONVERT(expr, type) MySQL two-argument syntax — `CONVERT(price, CHAR)` and `CONVERT('42', UNSIGNED INTEGER)` not in function dispatcher; only `CONVERT(expr USING charset)` is partially handled; fix: detect two positional args form and route to CAST path
- [ ] 4.19h ⏳ Math functions: PI, LOG, LOG2, LOG10, EXP, trig (SIN/COS/TAN/ATAN/ATAN2), RADIANS, DEGREES — not in `eval/functions/mod.rs` dispatcher; all return NotImplemented; one-liners using Rust f64::{ln, log2, log10, exp, sin, cos, tan, atan, atan2}; add to `eval/functions/numeric.rs`
- [ ] 4.19i ⏳ Date functions: SYSDATE vs NOW distinction, UTC_DATE/TIME/TIMESTAMP, FROM_UNIXTIME, UNIX_TIMESTAMP, DATEDIFF, CONVERT_TZ — missing from `eval/functions/datetime.rs`; SYSDATE() should return execution time (not statement start like NOW()); UTC_* return UTC equivalents; FROM_UNIXTIME(n) converts epoch to DATETIME; DATEDIFF(d1,d2) returns days between dates; CONVERT_TZ for timezone conversion
- [ ] 4.19j ⏳ String functions: NVL2, INSERT(str,pos,len,new), ORD, SOUNDEX, CHAR variadic — NVL2(e,t,f) is 3-arg null conditional; INSERT(str,pos,len,newstr) is string-replacement (not SQL INSERT); ORD(str) returns code point of first char; SOUNDEX phonetic encoding; CHAR(n1,n2,...) current impl is single-arg only, MySQL accepts variadic; add to respective function modules
- [ ] 4.1c ⏳ Column attributes parse errors — `UNSIGNED`, `COLLATE utf8mb4_unicode_ci`, `CHARACTER SET utf8mb4`, `ON UPDATE CURRENT_TIMESTAMP`, `COMMENT 'text'` after the column type all cause parse errors (`parser/ddl.rs:77`); present in virtually every production MySQL schema; fix: extend `parse_column_def` to consume and discard (or store) these tokens; `ON UPDATE CURRENT_TIMESTAMP` requires executor support to auto-refresh on row UPDATE
- [ ] 4.1d ⏳ CREATE TABLE inline INDEX/KEY — `INDEX idx (col)` and `KEY idx (col)` inside the column list cause parse errors (`parser/ddl.rs:68`); most MySQL schemas define non-unique indexes inline; fix: add INDEX/KEY to `is_table_constraint_start`, parse like CREATE INDEX on the same table
- [ ] 4.1b ⏳ CREATE TABLE table options — `ENGINE=InnoDB`, `DEFAULT CHARSET=utf8mb4`, `COLLATE=...`, `AUTO_INCREMENT=N`, `COMMENT=`, `ROW_FORMAT=`, `KEY_BLOCK_SIZE=` after the closing `)` currently cause a **parse error** (`parser/ddl.rs:58`); every `mysqldump` output uses these; fix: loop after `)` consuming `KEY=VALUE` pairs and discarding them; blocks importing any real MySQL schema dump
- [ ] 4.4b ⏳ Expression operators: REGEXP/RLIKE, XOR, DIV, <=>, bitwise — `REGEXP`/`RLIKE` (regex match), `XOR` (boolean), `DIV` (integer division), `<=>` (null-safe equality), `&`/`|`/`^`/`~`/`<<`/`>>` (bitwise) are not in the lexer or expression parser; `WHERE flags & 0x01 = 1` and `WHERE email REGEXP '^[a-z]+'` cause parse errors; critical for permission/bitmask queries and validation
- [ ] 4.4c ⏳ SET SESSION/GLOBAL + multi-variable SET + @user_var — `SET SESSION var = val` and `SET GLOBAL var = val` fail because SESSION/GLOBAL keywords are not consumed by the parser (`parser/mod.rs:380`); `SET a=1, b=2` (comma-separated) not parsed; `SET @user_var = expr` (single `@` user variables) not supported; JDBC drivers use `SET SESSION` exclusively for setting transaction isolation
- [ ] 4.4d ⏳ LIKE ESCAPE clause not parsed — `WHERE path LIKE 'reports\_%' ESCAPE '\'` causes a parse error; `ESCAPE` clause defines the escape character so literal `%` and `_` can appear in patterns; fix: add `escape: Option<Expr>` to the LIKE AST node; `eval/core.rs` `like_match` already accepts an escape char, just needs wiring from parser
- [ ] 4.3h ⏳ Standalone RENAME TABLE statement — `RENAME TABLE a TO b` and `RENAME TABLE a TO b, c TO d` (multi-pair atomic rename) not in Stmt enum; only ALTER TABLE RENAME exists; add RenameTable(Vec<(String,String)>) to AST + parser + executor; also fix single-pair break in ddl.rs:1711 that prevents multi-rename
- [ ] 4.3f ⏳ CREATE TABLE ... SELECT (CTAS) — `CREATE TABLE new SELECT * FROM src` not handled; parser at `ddl.rs:36` expects `(col_defs)` immediately; fix: after table name, branch on SELECT token to execute inner SELECT and derive columns/data
- [ ] 4.3g ⏳ CREATE TABLE ... LIKE — `CREATE TABLE staging LIKE users` copies schema without data; LIKE after table name not in parser (`ddl.rs:36`); fix: read source table name and copy its TableDef from catalog
- [ ] 4.4e ⏳ INSERT IGNORE not parsed — `INSERT IGNORE INTO t VALUES (...)` silences constraint violations (UNIQUE, FK, NOT NULL) and returns warning count instead of error; `parser/dml.rs:376`; add IGNORE token after INSERT; store `ignore: bool` in InsertStmt; executor converts DuplicateKey/NotNullViolation to warnings when flag set
- [ ] 4.4f ⏳ SELECT FOR UPDATE / LOCK IN SHARE MODE not parsed — `SELECT ... FOR UPDATE` and `SELECT ... LOCK IN SHARE MODE` cause parse errors (`parser/dml.rs:92`); used by JPA/Hibernate, GORM, hand-written transactions; fix: parse and store `lock_mode: Option<LockMode>` in SelectStmt; executor treats as normal SELECT until Phase 13.7 row-level locking
...
- [ ] 4.4h ⏳ CALL / DO statements not parsed — `CALL procedure(args)` and `DO expr` not in Token enum or top-level parser dispatch (`parser/mod.rs:227`); CALL is needed to not break apps using stored procedures; DO is MySQL's expression-evaluate-and-discard; fix: parse both and return NotImplemented (CALL) or silently succeed (DO)
- [ ] 4.4i ⏳ SELECT HIGH_PRIORITY / STRAIGHT_JOIN modifiers not consumed
- [ ] 4.4j ⏳ UPDATE SET col = DEFAULT not parsed — `UPDATE t SET stock = DEFAULT` should reset column to its default value; parser treats DEFAULT as an identifier in this context (`parser/dml.rs:462`); fix: emit Expr::Default when DEFAULT token appears after `=` in assignment; executor resolves same as INSERT DEFAULT VALUES — `SELECT HIGH_PRIORITY ...` and `SELECT STRAIGHT_JOIN ...` cause parse errors because these optimizer hint keywords are not consumed between SELECT and the column list (`parser/dml.rs:50`); also: SQL_SMALL_RESULT, SQL_BIG_RESULT, SQL_BUFFER_RESULT; fix: consume and discard these tokens
- [ ] 4.5d ⏳ BEGIN WORK + START TRANSACTION READ ONLY/READ WRITE — `BEGIN WORK` leaves `WORK` token unparsed → parse error; `START TRANSACTION READ ONLY` and `READ WRITE` similarly fail (`parser/mod.rs:354-363`); fix: consume optional `WORK` keyword; parse optional `READ ONLY` / `READ WRITE` modifiers and store in session for the next transaction
- [ ] 4.20c ⏳ INFORMATION_SCHEMA virtual database — `SELECT ... FROM information_schema.tables/columns/statistics/key_column_usage/referential_constraints` returns table-not-found; all major ORMs (Sequelize, TypeORM, Prisma, ActiveRecord, GORM, Hibernate) query it on connect for schema discovery; minimum virtual tables: TABLES, COLUMNS, KEY_COLUMN_USAGE, REFERENTIAL_CONSTRAINTS, TABLE_CONSTRAINTS, STATISTICS
- [ ] 4.22e ⏳ ALTER TABLE DROP/MODIFY COLUMN auto-index-drop — `DROP COLUMN` and `MODIFY COLUMN` return NotImplemented if the column is part of an index (`ddl.rs:1858`, `ddl.rs:1944`); MySQL auto-drops or rebuilds the index as part of the operation; fix: detect affected indexes, drop them, proceed with the column op, then rebuild if MODIFY
- [ ] 4.6b ⏳ INSERT INTO t SET col=val syntax — MySQL-specific `INSERT SET` syntax not in parser (`InsertSource::Set` missing from `ast.rs`); `INSERT INTO t SET name='Alice', score=100` used by many MySQL ORMs and legacy code; parse as a single-row VALUES
- [ ] 4.6c ⏳ INSERT ... VALUES (1, DEFAULT) — `DEFAULT` as a value in VALUES list is not an `Expr` variant; `INSERT INTO t (a, b) VALUES (1, DEFAULT)` fails; add `Expr::Default` and resolve to column default in executor
- [ ] 4.6d ⏳ INSERT column count mismatch must error — `INSERT INTO t (a, b) VALUES (1)` silently pads missing columns with NULL instead of returning error 1136 "Column count doesn't match value count" (`insert.rs:1622`); fix: after building the column map, assert `values.len() == col_map.len()` and return `DbError::ColumnCountMismatch`
- [ ] 4.1g ⏳ CREATE DATABASE IF NOT EXISTS / CHARACTER SET / COLLATE — extra tokens after database name cause parse errors (`parser/ddl.rs:19`); `mysqldump` generates `CREATE DATABASE IF NOT EXISTS db DEFAULT CHARACTER SET utf8mb4`
- [ ] 4.21b ⏳ HAVING clause alias resolution — `HAVING cnt > 5` where `cnt` is an alias from the SELECT list is not resolved; must repeat expression `HAVING COUNT(*) > 5`; MySQL and PostgreSQL both support alias in HAVING; fix: build alias map from SELECT projections and substitute in HAVING expr
- [ ] 4.22h ⏳ ALTER TABLE ADD INDEX / DROP INDEX / CHANGE COLUMN / AUTO_INCREMENT=N — not in `AlterTableOp` enum; `ADD INDEX idx (col)` and `ADD UNIQUE INDEX` are how most migration tools add indexes (not via separate CREATE INDEX); `CHANGE COLUMN old new type` is MySQL's rename+retype in one op; `AUTO_INCREMENT = N` resets the sequence counter; `ALTER COLUMN SET DEFAULT` / `DROP DEFAULT` changes column defaults
- [ ] 4.1e ⏳ SET FOREIGN_KEY_CHECKS=0 / UNIQUE_CHECKS=0 — `mysqldump` wraps every import with these; if unrecognized they fail the import; `FOREIGN_KEY_CHECKS=0` should suppress FK enforcement on the current connection; `UNIQUE_CHECKS=0` should suppress unique constraint checking; add to handled session variables in `session.rs`; wire into executor for FK path
- [ ] 4.22g ⏳ ALTER TABLE RENAME INDEX — `ALTER TABLE t RENAME INDEX old TO new`; not in `AlterTableOp` enum (`ast.rs:368`); needed by migration tools that rename indexes without dropping them; executor: update index name in `axiom_indexes` catalog, no data movement
- [ ] 4.22i ⏳ ALTER TABLE t CONVERT TO CHARACTER SET — `ALTER TABLE users CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`; not in AlterTableOp enum (`ast.rs:345`); common in latin1→utf8mb4 migrations; add ConvertCharset variant and catalog update logic
...
- [ ] 4.17e ⏳ Float division by zero returns Infinity instead of NULL — `1.0 / 0` gives ±Inf in AxiomDB, MySQL returns NULL; `eval/ops.rs:216` performs IEEE 754 division without a zero check; fix: add `if b == 0.0 { return Ok(Value::Null); }` before float division
- [ ] 4.17d ⏳ IS TRUE / IS FALSE / IS NOT TRUE / IS NOT FALSE predicates — not in IS-clause parser (`parser/expr.rs:120`); `WHERE flag IS TRUE` cause parse errors; fix: after IS [NOT], check for TRUE/FALSE tokens; IS TRUE ≡ =1, IS NOT TRUE ≡ <>1 OR IS NULL
...
- [ ] 4.14b ⏳ LAST_INSERT_ID(expr) — the 1-argument form `LAST_INSERT_ID(expr)` evaluates `expr`, stores it as the session last-insert-id, and returns it; `system.rs:19` currently ignores args; used for manual sequence tracking patterns common in MySQL codebases
- [ ] 4.5e ⏳ SQL_CALC_FOUND_ROWS / FOUND_ROWS() — `SELECT SQL_CALC_FOUND_ROWS * FROM t LIMIT 10` stores the pre-LIMIT count; `FOUND_ROWS()` returns it; legacy MySQL pagination idiom used in millions of existing apps (deprecated in MySQL 8.0.17 but still common); parser: consume and discard the modifier; executor: stash pre-limit count in session; evaluator: `found_rows()` reads it
- [ ] 4.5f ⏳ AUTO_INCREMENT explicit 0 treated as manual insert — MySQL treats `INSERT INTO t (id) VALUES (0)` the same as NULL for AUTO_INCREMENT columns (assigns next sequence value); AxiomDB stores the literal `0`; fix: in executor/insert.rs, when column has auto_increment flag and supplied value is `Value::Int(0)`, substitute with next sequence value
- [ ] 5.11d ⏳ COM_CHANGE_USER (0x11) — JDBC connection pools (HikariCP, c3p0) send this to recycle connections; handler has no case for command byte 0x11 → connection closes or hangs; fix: reset session state and return OK (like COM_RESET_CONNECTION)
- [ ] 5.10b ⏳ Prepared statement param count not validated — param_count not checked against 65535 limit in parse_execute_packet() (`prepared.rs:128`); malformed packet declaring 100K params causes large alloc; add guard before null-bitmap processing
- [ ] 5.4c ⏳ COM_STATISTICS (0x09) stub — monitoring agents and legacy MySQL clients send this and expect a plain-text statistics string (uptime, threads, etc.); handler currently returns error 1047; fix: add `0x09 =>` case returning a minimal statistics string without closing the connection
- [ ] 5.4b ⏳ COM_* unhandled commands close connection — unknown COM bytes (COM_STATISTICS 0x09, COM_PROCESS_INFO 0x0a, COM_DEBUG 0x0d, COM_FIELD_LIST 0x04, COM_REFRESH 0x07, COM_SHUTDOWN 0x08, COM_BINLOG_DUMP 0x12, COM_STMT_FETCH 0x1c) fall through to error 1047 and may close the connection (`handler.rs:1356`); MySQL stays connected and returns graceful error; fix: add match arms returning ERR 1047 while keeping connection alive
- [ ] 5.9i ⏳ Session variables return None for common ORM capability queries
- [ ] 5.9j ⏳ Missing session variables: character_set_system, init_connect, collation_server — return None from get_variable() (`session.rs:584`); add hardcoded defaults: character_set_system→"utf8mb4", init_connect→"", collation_server→"utf8mb4_0900_ai_ci"
- [ ] 5.9k ⏳ @@global.var / SET @@global.var not handled — get_variable() strips @@session. and @@ but not @@global. prefix (`session.rs:584`); apply_set() same gap; SELECT @@global.max_allowed_packet returns None; fix: strip @@global. same as @@session.
- [ ] 5.9l ⏳ SET CHARACTER SET charset not handled — distinct from SET NAMES; sets only client_charset without changing connection collation (`session.rs:343`); JDBC uses this form; add branch in apply_set() for CHARACTER SET keyword sequence — `performance_schema`, `have_query_cache`, `log_bin`, `net_buffer_length`, `transaction_read_only`, `sql_mode` all return None from `get_variable()` (`session.rs:584`); ORMs query these on connect; fix: add safe defaults before the fallthrough
...
- [ ] 5.9f ⏳ SHOW FULL TABLES / SHOW FULL COLUMNS / SHOW TABLE STATUS — `SHOW FULL TABLES` adds `Table_type` column (Sequelize/ActiveRecord use it); `SHOW FULL COLUMNS FROM t` adds Privileges+Comment (Prisma/TypeORM use it); `SHOW TABLE STATUS` returns row count, engine, charset per table; parser doesn't handle FULL modifier or TABLE STATUS
- [ ] 5.9g ⏳ SHOW ENGINES / SHOW CHARSET / SHOW COLLATION — DB management tools (Workbench, DBeaver, TablePlus) probe these on connect; return single static rows: one InnoDB-compatible engine row, utf8mb4/utf8/latin1 charsets, standard collation list
- [ ] 5.9e ⏳ SHOW WARNINGS / SHOW ERRORS — MySQL connectors (JDBC, MySQL Connector/Python, mysqlclient) issue `SHOW WARNINGS` after every DML statement; warning_count is already set in OK packets (5.9b) but `SHOW WARNINGS` itself is not in the parser; requires: `ShowWarningsStmt` / `ShowErrorsStmt` AST nodes + parser dispatch + per-session `Vec<Warning>` in `SessionContext` filled by executor on truncation, coercion, FK soft-ignore, and similar non-fatal conditions; `SHOW WARNINGS LIMIT N` variant
...
- [ ] 6.6b ⏳ ON UPDATE CASCADE / ON UPDATE SET NULL — FK update actions beyond `RESTRICT`/`NO ACTION` return `NotImplemented` (`fk_enforcement.rs:776`); requires walking child rows on UPDATE and propagating the new PK value or setting NULL; ON DELETE CASCADE already works (Phase 6.6); this is the symmetric UPDATE side
...
- [ ] 4.1f ⏳ Version-conditional MySQL comments (/*!...*/`) — `mysqldump` wraps version-specific SQL in `/*!50600 ... */`; parser currently strips all block comments losing the code inside; fix: detect `/*!` prefix in lexer, execute content when embedded version ≤ 80000 (MySQL 8)
...
- [ ] 4.6e ⏳ INSERT LOW_PRIORITY / HIGH_PRIORITY / DELAYED — modifiers before INTO not in lexer; `INSERT LOW_PRIORITY INTO t VALUES (1)` causes parse error; parse and discard (semantics not needed for single-server use)
...
- [ ] 40.4b ⏳ Per-connection TxnState — move `active: Option<ActiveTxn>` out of `TxnManager` into per-connection `ConnectionTxn`; `TxnManager` becomes `TxnCoordinator` with `AtomicU64` for `next_txn_id`/`max_committed` + `RwLock<HashSet<TxnId>>` active set; `begin()` returns `ConnectionTxn` to caller; `commit()`/`rollback()` take `ConnectionTxn`; prerequisite for 40.10 (concurrent connections); spec: `specs/fase-40/spec-40.2-per-connection-txn.md`
- [ ] 40.5 ⏳ Lock Manager — row-level S/X/IS/IX intent locks; deadlock-safe wait queues; per-table lock table; `lock_mgr.acquire_table()` + `acquire_row()` API; spec: `specs/fase-40/spec-40.5-lock-manager.md`
- [ ] 40.6 ⏳ Deadlock Detection — wait-for graph, cycle detection (DFS), victim selection (youngest txn), `DbError::Deadlock`; spec: `specs/fase-40/spec-40.6-deadlock-detection.md`
- [ ] 40.7 ⏳ HeapChain Concurrent Access — page X-latch on INSERT/DELETE/UPDATE (PageLockTable already in 40.3); `HeapInsertHint` (AtomicU64 last page with space); atomic chain growth; MVCC reader without page latch; spec: `specs/fase-40/spec-40.7-heap-concurrent.md`
- [ ] 40.8 ⏳ B-tree Latch Coupling — SMO-safe latch-coupling for concurrent B-tree traversal; optimistic → pessimistic path; spec: `specs/fase-40/spec-40.8-btree-latch-coupling.md`
