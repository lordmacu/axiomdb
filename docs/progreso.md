...
>
> **MySQL+PG parity path: 145/442 subphases (32.8%) — Phase 11 complete; Phase 13 complete**
>
...
2. **Fase 13 Advanced PostgreSQL** — window funcs, generated cols, triggers BEFORE/AFTER, collation ICU.
3. **Fase 20 Types + import/export** — views, sequences, ENUMs, arrays, COPY.
4. **Fase 16 Server logic** — stored procs, triggers completos, pooler.
5. **Fase 17 Security** — auth, TLS, RBAC, RLS, audit, TDE.
6. **Fase 19 Observability** — pg_stat_*, auto-vacuum, Prometheus, health.
7. **Fase 18 High availability** — streaming replication, PITR, backup.
8. **Fase 24-26 Type system complete** — complete types + optimizations + full collation.
9. **Fase 27 Real query optimizer** — DP, cardinality, adaptive.
10. **Fase 28-30 SQL pro + functions + infrastructure**.
11. **Fase 12 + 31 + 32 + 35 Polish + deploy** — testing, final features, architecture, DevEx.
...
| C. Type System + SQL completeness | 11, 13, 20, 21, 24, 25, 26 | 🔄 fase 11 complete; fase 13 complete; fase 21 next |
...
- [ ] 11.18 ⏳ PostgreSQL JSONB operator parity — split into sub-phases:
  ...
  - [x] ✅ 11.18b (partial) — `?|` (any-of) and `?&` (all-of) JSONB key-existence operators with **JSONB-array RHS** (documented PG divergence: PG accepts `text[]`, AxiomDB accepts a JSONB string array — `'["a","b"]'::jsonb`). New `Token::JsonExistsAny` / `Token::JsonExistsAll` (must precede `?` in lexer order); new `BinaryOp::JsonExistsAny` / `BinaryOp::JsonExistsAll` threaded through `expr_to_sql` + DDL printer. Evaluator `eval_jsonb_exists_set(left, right, all)` walks the RHS JSONB array, filters to string elements, and applies any-of / all-of against object keys (or string array elements) of the LHS doc. Empty RHS array → all-of vacuously true / any-of false. NULL on either side → NULL. Non-array RHS → TypeMismatch. 9 integration tests in `tests/integration_jsonb_exists_set.rs` (any/all hits & misses, array-doc LHS, empty-array semantics, NULL propagation both sides, WHERE-clause row filter). The remaining path/delete operators moved to `11.18c`, which follows the same documented JSONB-array RHS divergence instead of blocking on native `TEXT[]`.
  - [x] ✅ 11.18c — PostgreSQL JSONB path operators `#-`, `#>`, `#>>` closed with **JSONB-array RHS** (documented PG divergence: PostgreSQL accepts `text[]`, AxiomDB accepts a JSONB string array path like `CAST('[\"a\",1]' AS JSONB)`). `Token::JsonPathExtract` / `Token::JsonPathExtractText` / `Token::JsonPathDelete` live in the lexer; the `#` line-comment strip rule now only applies at line start so operator tokens and JSON strings survive tokenization. `eval_jsonb_path_extract` / `eval_jsonb_path_delete` in `eval/ops.rs` reuse `jsonb_path_segments` + `descend_serde` / `prune_serde` to cover nested object extraction, array index extraction, text rendering, nested delete, missing-path no-op, and NULL propagation. Covered by 10 integration tests in `tests/integration_jsonb_path_ops.rs` plus wire smoke. Completed 2026-04-22.
...
- [ ] 11.21 ⏳ JSONPath parity + indexed path operators — split into sub-phases:
  ...
  - [x] ✅ 11.21h — Planner predicate extraction for simple indexable JSONPath over the existing `jsonb_ops`-style GIN index. `planner_select::plan_gin_scan` now recognizes bounded literal forms `doc @? '$.k'` and `doc @@ '$.flag'` on base JSONB columns, emits `AccessMethod::GinScan`, and reuses `gin_key_term(...)` for the top-level key probe. Scope is intentionally conservative: only simple `$.key` literals are extracted; nested paths, filters, array steps, accessors, arithmetic, and function-call forms still fall back to scan. Executor semantics stay unchanged — `GinScan` remains a candidate filter and the original predicate is rechecked, so `@@ '$.flag'` can probe by key while still excluding `{\"flag\":false}` at evaluation time. Coverage: new `tests/integration_jsonpath_planner_pushdown.rs` (5 tests for `@?`, `@@`, fallback without index, fallback on nested path, and recheck correctness) plus wire smoke `442/442` with a dedicated `[11.21h JSONPath planner pushdown]` block. `jsonb_path_ops` remains deferred as future storage/opclass work. Completed 2026-04-22.
...
- [ ] 11.25 ⏳ JSON SRF + aggregates + construction helpers (PG paridad "streaming") — cerrar la brecha de *unnesting/construction* idiomatico de PG usado en producciones reales (`jsonb_each`, `jsonb_agg`, `jsonb_build_object`, etc.). Split:
...
- [x] 13.5 ✅ Covering indexes — closed: heap secondary index leaves now persist `INCLUDE (...)` payload values alongside the logical key, and `IndexOnlyScan` can decode covered non-key projections without touching the heap. Planner coverage now treats `include_columns` as real coverage, write-path maintenance updates/deletes the payload correctly, and old pre-13.5 entries remain removable/readable through legacy-key fallback paths. FK parent lookups and other logical-key probes were updated to avoid exact-key assumptions on INCLUDE-backed unique indexes. Coverage: `crates/axiomdb-sql/tests/integration_index_only.rs`, `crates/axiomdb-sql/tests/integration_fk.rs`, `crates/axiomdb-sql/tests/integration_executor_ddl.rs`, wire smoke `[13.5 covering indexes]` in `tools/wire-test.py` (453/453).
- [x] 13.6 ✅ Non-blocking ALTER TABLE — closed with bounded heap shadow-rebuild path: single-op `ADD/DROP/MODIFY COLUMN` on heap tables now copies into a shadow root under shared catalog lock, swaps metadata atomically in a short final exclusive window, keeps readers alive during copy, and rejects concurrent writers to the target table with `LockTimeout`; full WAL-delta replay / concurrent-writer online DDL stays deferred
...
- [x] 13.12 ✅ Statement-level triggers — closed with a bounded validation-trigger MVP: `CREATE TRIGGER ... AFTER INSERT|UPDATE|DELETE ... FOR EACH STATEMENT AS SELECT ...`, `DROP TRIGGER ... ON table`, and `SHOW CREATE TRIGGER ... ON table` now work on base tables. Trigger metadata is table-owned in catalog, fires once after top-level DML, and aborts the outer statement when the validation `SELECT` returns rows. Built-in trigger context exposes `@@trigger_name`, `@@trigger_table`, `@@trigger_event`, and `@@trigger_row_count` via SQL-text substitution inside the stored body. Coverage: `crates/axiomdb-sql/tests/integration_statement_triggers.rs`, parser coverage in `crates/axiomdb-sql/tests/integration_ddl_parser.rs`, wire smoke block `[13.12 statement triggers]` in `tools/wire-test.py` (461/461). Deferred to Phase 16: `BEFORE`, `FOR EACH ROW`, `WHEN`, `SIGNAL`, transition tables, recursive triggers, and procedural bodies
- [x] 13.13 ✅ Collation system — closed with a bounded layered collation slice over the existing runtime modes. Catalog now persists optional collations on databases/tables/columns; DDL normalizes MySQL-facing aliases onto canonical `binary` / `es`; binder/evaluator support inherited column collations plus query-level `expr COLLATE ...` on current collation-aware paths; and metadata/reporting (`SHOW CREATE TABLE`, `SHOW FULL COLUMNS`, `SHOW TABLE STATUS`, `information_schema.TABLES`, `information_schema.COLUMNS`) now reflects effective collations instead of hardcoded values. Public docs were corrected to remove the stale claim that UCA root / full ICU was already implemented. Coverage: `integration_mysql_compat.rs`, `integration_g9_ddl.rs`, `integration_show_full.rs`, `integration_collation_system.rs`. Deferred: full ICU/UCA locale tailoring, larger collation registry, `COLLATE 'auto'`, and broader executor/index coverage tracked under later collation phases.
- [x] 13.14 ✅ Custom aggregate functions — closed 2026-04-27: `CREATE AGGREGATE name(type) (SFUNC=..., STYPE=..., FINALFUNC=...)` backed by a bounded internal registry; catalog persists `AggregateDef` with binary serialization; analyzer resolves custom aggregate calls by name + arity before built-ins; executor dispatches through `AggregateHelperKind` registry; `DROP AGGREGATE` removes the catalog entry. Ships `median(FLOAT)` as the first real custom aggregate (exact sort-based). Coverage: `tests/integration_custom_aggregates.rs` (5 tests), `integration_ddl_parser.rs` (2 tests), wire smoke `[13.14 custom aggregate functions]` in `tools/wire-test.py` (465/465). Deferred: multi-arg signatures, arbitrary SFUNC/FINALFUNC SQL bodies, window execution, ordered-set aggregates, `INITCOND`/`COMBINEFUNC`.

### Phase 20 — Types + import/export `⏳` week 47-48
- [x] 20.1 ✅ Regular views — closed 2026-04-27: `CREATE VIEW`, `CREATE OR REPLACE VIEW`, and `DROP VIEW [IF EXISTS]` with catalog support (`RelationKind::View`, no physical pages); transparent view expansion at analysis time via `expand_views()` mirroring the CTE pattern; `SHOW CREATE VIEW`; `information_schema.VIEWS` virtual table; `SHOW TABLES` / `information_schema.TABLES` report `TABLE_TYPE = 'VIEW'`; `execute_select_derived` extended to handle JOIN on derived-table FROM (fixing Subquery+JOIN routing). Coverage: `crates/axiomdb-sql/tests/integration_views.rs` (16 tests), wire smoke `[20.1 regular views]` in `tools/wire-test.py` (473/473). Deferred: updatable views (INSERT/UPDATE/DELETE through a view), WITH CHECK OPTION, column-name alias lists, security-definer/invoker, `SHOW FULL TABLES` view filter.
...
- [x] 21.10 ✅ Cursors — `DECLARE`, `FETCH`, `CLOSE` — completed 2026-04-21
- [x] 21.11 ✅ Query hints — bounded `SELECT /*+ ... */` MVP completed: lexer preserves optimizer comments as `Token::OptimizerHint`, parser stores `SelectStmt.hints`, `INDEX(table index)` can steer single-table planner selection when compatible, `HASH_JOIN` overrides the small-join threshold for eligible equijoins, `PARALLEL(n)` is accepted as advisory metadata and surfaced via `EXPLAIN`, and wire smoke adds `21.11` coverage. Joined `INDEX(...)` hints remain explicitly deferred beyond the single-table MVP. Completed 2026-04-22.
...
- [x] 21.16 ✅ DEFERRABLE constraints — bounded FK-only MVP: parser/catalog now persist `DEFERRABLE` + `INITIALLY DEFERRED|IMMEDIATE`, `DEFERRABLE INITIALLY DEFERRED` skips statement-time FK rejection and revalidates touched FK definitions on `COMMIT`, failed commit-time validation rolls the full transaction back, and savepoint rollback truncates pending deferred-FK state correctly. `SET CONSTRAINTS`, deferred CHECK, and deferred exclusion remain explicitly out of scope. Completed 2026-04-22.
...
- [x] 21.20 ✅ CHECKPOINT — real SQL admin statement wired to the existing checkpoint engine; persists a checkpoint LSN, rejects while any transaction is active, and runs outside implicit autocommit transactions — completed 2026-04-22
...
- [x] 21.23 ✅ Advanced SQL tests — new acceptance suite `integration_advanced_sql.rs` covers non-recursive + recursive CTEs, `MERGE` + savepoints, cursors over CTE-backed queries, `CHECKPOINT`, and grouping sets in multi-step session flows; `tools/wire-test.py` adds a `MERGE` + `SAVEPOINT` protocol smoke; stale `window functions` wording removed because `OVER (...)` is not implemented yet in this codebase. Completed 2026-04-22.
- [x] 21.25 ✅ PIVOT dynamic — bounded `PIVOT` landed as a real `FROM` operator with explicit `IN (...)` values, single-aggregate MVP, analyzer-time schema derivation, and lowering to grouped `CASE WHEN` aggregation via derived subquery rewrite; supports outer projection/JOIN usage and wire-visible smoke coverage, while true runtime-discovered columns and `UNPIVOT` stay deferred. Completed 2026-04-22.
- [x] 21.24 ✅ ORM compatibility tier 2 — bounded compatibility baseline closed: Prisma/ActiveRecord-style connect + metadata probes now cover `SHOW FULL FIELDS`, `SHOW FULL TABLES`, `SHOW TABLE STATUS`, `SHOW CREATE TABLE`, no-op dump-style `SET` toggles, and migration-style `INSERT ... RETURNING` over the MySQL wire; `GENERATED ALWAYS AS IDENTITY` remains deferred to `24.1c` — completed 2026-04-22
