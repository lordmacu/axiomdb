...
| `NEXTVAL('seq')`        | `BIGINT`    | Advances a named sequence and returns the consumed value |
| `CURRVAL('seq')`        | `BIGINT`    | Returns this session's last value for a named sequence |
...
SELECT CURRVAL('invoice_seq'); -- 1000
```

`CURRVAL` is session-local and errors until the same connection has called
`NEXTVAL` for that sequence. `NEXTVAL` changes are durable sequence state and
are not undone by transaction rollback, so gaps are expected.
