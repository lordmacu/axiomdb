...
# ── Phase 24.1c — GENERATED ALWAYS|BY DEFAULT AS IDENTITY ────────────────────

# ALWAYS: sequence assigns, explicit values rejected.
cur.execute("DROP TABLE IF EXISTS wire_identity")
cur.execute("CREATE TABLE wire_identity (id INT GENERATED ALWAYS AS IDENTITY, v TEXT)")
cur.execute("INSERT INTO wire_identity(v) VALUES ('hello')")
cur.execute("INSERT INTO wire_identity(v) VALUES ('world')")
cur.execute("SELECT id, v FROM wire_identity ORDER BY id")
rows = list(cur.fetchall())
ok("[24.1c identity_always] auto-assigns sequential IDs",
   rows == [(1, 'hello'), (2, 'world')])

# Reject explicit value.
try:
    cur.execute("INSERT INTO wire_identity(id, v) VALUES (99, 'rejected')")
    rejected = False
except Exception:
    rejected = True
ok("[24.1c identity_always_rejects_explicit] explicit INSERT into ALWAYS column errors",
   rejected)

# SHOW COLUMNS reports identity syntax.
cur.execute("SHOW COLUMNS FROM wire_identity")
cols = cur.fetchall()
id_extra = next((r[5] for r in cols if r[0] == 'id'), None)
ok("[24.1c identity_show_columns] SHOW COLUMNS reports GENERATED ALWAYS AS IDENTITY",
   id_extra == 'GENERATED ALWAYS AS IDENTITY')

# BY DEFAULT: explicit values accepted, NULL still autogenerates.
cur.execute("DROP TABLE IF EXISTS wire_identity_bd")
cur.execute("CREATE TABLE wire_identity_bd (id BIGINT GENERATED BY DEFAULT AS IDENTITY, v TEXT)")
cur.execute("INSERT INTO wire_identity_bd(v) VALUES ('auto')")
cur.execute("INSERT INTO wire_identity_bd(id, v) VALUES (100, 'explicit')")
cur.execute("INSERT INTO wire_identity_bd(id, v) VALUES (NULL, 'also-auto')")
cur.execute("SELECT id, v FROM wire_identity_bd ORDER BY id")
bd_rows = cur.fetchall()
ok("[24.1c identity_by_default] BY DEFAULT accepts explicit + autogenerates on NULL",
   bd_rows[0][1] == 'auto' and any(r[0] == 100 for r in bd_rows))
